...
#ifndef BASIC_EC_RANKS_PER_GROUP
#define BASIC_EC_RANKS_PER_GROUP 4  // Ranks per BasicEC group (multi-rank support)
#endif
...
private:
    ...
    std::atomic<bool> basic_ec_load_recv_rank3_data1_connected_{false};
    std::atomic<bool> basic_ec_load_send_rank3_data1_connected_{false};
...
public:
    AsioConnectionManager()
        : io_context_(),
          ...
          basic_ec_load_recv_rank3_data1_socket_(io_context_),
          basic_ec_load_recv_rank3_data1_acceptor_(io_context_),
          basic_ec_load_send_rank3_data1_socket_(io_context_) {}
    ...
    boost::asio::ip::tcp::socket& get_basic_ec_load_recv_rank3_data1_socket() {
        return basic_ec_load_recv_rank3_data1_socket_;
    }
    boost::asio::ip::tcp::socket& get_basic_ec_load_send_rank3_data1_socket() {
        return basic_ec_load_send_rank3_data1_socket_;
    }
    ...
    bool is_basic_ec_load_send_rank3_data1_connected() const {
        return basic_ec_load_send_rank3_data1_connected_;
    }
...
void AsioConnectionManager::bind_listen_basic_ec_load_recv_rank3_data1(const std::string& listen_ip, uint16_t port) {
    try {
        boost::asio::ip::tcp::endpoint endpoint(boost::asio::ip::address::from_string(listen_ip), port);
        basic_ec_load_recv_rank3_data1_acceptor_.open(endpoint.protocol());
        basic_ec_load_recv_rank3_data1_acceptor_.set_option(boost::asio::ip::tcp::acceptor::reuse_address(true));
        basic_ec_load_recv_rank3_data1_acceptor_.bind(endpoint);
        basic_ec_load_recv_rank3_data1_acceptor_.listen();
    } catch (const std::exception& e) {
        std::cerr << "BasicEC: bind_listen_basic_ec_load_recv_rank3_data1 error: " << e.what() << std::endl;
        throw;
    }
}
...
void AsioConnectionManager::accept_basic_ec_load_recv_rank3_data1() {
    try {
        ...
        basic_ec_load_recv_rank3_data1_connected_ = true;
        connection_cv_.notify_all();
    } catch (const std::exception& e) {
        std::cerr << "BasicEC: accept_basic_ec_load_recv_rank3_data1 error: " << e.what() << std::endl;
        basic_ec_load_recv_rank3_data1_connected_ = false;
        connection_cv_.notify_all();
    }
}
...
void AsioConnectionManager::init_basic_ec_load_send_rank3_data1(const std::string& rank2_ip, uint16_t port) {
    try {
        ...
        std::cerr << "BasicEC: init_basic_ec_load_send_rank3_data1 error: " << e.what() << std::endl;
        basic_ec_load_send_rank3_data1_connected_ = false;
        connection_cv_.notify_all();
        throw;
    }
}
...
void AsioConnectionManager::cleanup() {
    ...
    if (basic_ec_load_recv_rank3_data1_socket_.is_open()) basic_ec_load_recv_rank3_data1_socket_.close();
    if (basic_ec_load_recv_rank3_data1_acceptor_.is_open()) basic_ec_load_recv_rank3_data1_acceptor_.close();
    if (basic_ec_load_send_rank3_data1_socket_.is_open()) basic_ec_load_send_rank3_data1_socket_.close();
}
...
struct EcRsPoolWorkerCtx {
    BasicECNative* self{nullptr};
    int wid{0};
};

class BasicECNative {
public:
    BasicECNative(const std::vector<std::string>& send_ips,
        ...
        if (static_cast<int>(send_ips.size()) != num_channels_ ||
            ...
            throw std::runtime_error("BasicECNative: Mismatched channel counts");
        }
        if (num_channels_ != n_ - 1) {
            throw std::runtime_error("BasicECNative: Expected n-1 channels, got " +
                                     std::to_string(num_channels_));
        }
        if (k_ < 2) {
            throw std::runtime_error("BasicECNative: k must be >= 2");
        }
        ...
        if (use_rdma_) {
            try {
                ...
                std::cerr << "BasicEC: RDMA initialization failed: " << e.what() << std::endl;
                std::cerr << "BasicEC: Falling back to ASIO" << std::endl;
                use_rdma_ = false;
            }
        }
    ...
    void submit_basic_ec_save(uintptr_t data0_addr,      // d_{i0} - keep, not sent
    ...
    void submit_basic_ec_decode_recovery(
            int k_orig, int m,
    ...
    {
        if (k_orig < 2 || m < 1 || m > 2) {
            throw std::invalid_argument("BasicEC decode: require k >= 2 and 1 <= m <= 2");
        }
        if (static_cast<int>(lost_positions.size()) != m ||
            static_cast<int>(recovered_addrs.size()) != m) {
            throw std::invalid_argument("BasicEC decode: lost and recovered lists must have length m");
        }
        if (static_cast<int>(surviving_rows.size()) != k_orig ||
            static_cast<int>(surviving_addrs.size()) != k_orig) {
            throw std::invalid_argument("BasicEC decode: exactly k ordered survivors are required");
        }
        std::vector<bool> lost_seen(k_orig, false);
        for (int pos : lost_positions) {
            if (pos < 0 || pos >= k_orig || lost_seen[pos]) {
                throw std::invalid_argument("BasicEC decode: lost data rows must be unique and in range");
            }
            lost_seen[pos] = true;
        }
        ...
        for (int row : surviving_rows) {
            if (row < 0 || row >= k_orig + 2 || survivor_seen[row] ||
                ...
                throw std::invalid_argument("BasicEC decode: survivor rows must be unique, valid, and not lost");
            }
            survivor_seen[row] = true;
        }
        if (size > static_cast<size_t>(std::numeric_limits<int>::max())) {
            throw std::invalid_argument("BasicEC decode: stripe size exceeds ISA-L int limit");
        }
        for (uintptr_t addr : surviving_addrs) {
            if (addr == 0) throw std::invalid_argument("BasicEC decode: null survivor address");
        }
        for (uintptr_t addr : recovered_addrs) {
            if (addr == 0) throw std::invalid_argument("BasicEC decode: null recovery address");
        }
        ...
        if (ret != 0) {
            throw std::runtime_error(
                "BasicEC decode: ISA-L gf_invert_matrix failed with code " +
                std::to_string(ret));
        }
    ...
    void sw_send_data(int block_idx, uintptr_t send_addr, size_t size) {
        ...
        std::cerr << "BasicEC: sw_send_data(" << block_idx << ", " << size << "): no channel" << std::endl;
    }
    ...
    void sw_recv_data(int block_idx, uintptr_t recv_addr, size_t size) {
        ...
        if (block_idx == 0 && conn_.is_basic_ec_load_recv_rank3_data1_connected()) {
            if (!recv_with_size_bool(conn_.get_basic_ec_load_recv_rank3_data1_socket(),
                ...
                std::cerr << "BasicEC: sw_recv_data(0) legacy recv failed" << std::endl;
            }
            return;
        }
        std::cerr << "BasicEC: sw_recv_data(" << block_idx << ", " << size << "): no channel" << std::endl;
    }
    ...
    void set_load_mode(bool is_load, int failed_rank, int rank = -1, bool is_software_only = false) {
        ...
        failed_rank_in_group_ = (failed_rank >= 0)
            ? (failed_rank % BASIC_EC_RANKS_PER_GROUP)
            : -1;
        if (rank >= 0) {
            rank_ = rank;
        }
    ...
    void init_basic_ec_load_connections_software_only(
        ...
        if (use_rdma_ && (rank_ == 2 || rank_ == 3)) {
            try {
                ...
                std::cerr << "BasicEC: Software-only load RDMA init failed: " << e.what() << std::endl;
                throw;
            }
        }
    }
    ...
    void init_basic_ec_load_sw_bind_listen(
    ...
    void init_basic_ec_load_sw_connect_one(
        int rank_in_group, const std::string& receiver_ip,
        int block_idx, uint16_t port) {
        if (rank_in_group == 2) return;  // receivers use init_basic_ec_load_sw_accept instead
        if (block_idx < 0 || block_idx >= sw_recovery_num_blocks_) {
            std::cerr << "BasicEC: SW recovery connect_one: block_idx " << block_idx
        ...
        if (use_rdma_) {
            if (!rdma_pd_) {
                if (!rdma_context_) {
                    if (ibv_fork_init() != 0) {
                        std::cerr << "[BasicEC RDMA] WARNING: ibv_fork_init() failed." << std::endl;
                    }
                    int num_devices;
                    ibv_device** device_list = ibv_get_device_list(&num_devices);
                    if (device_list && num_devices > 0) {
                        rdma_context_ = ibv_open_device(find_rdma_device_by_ip(
                            recv_ips_[0], device_list, num_devices, {"BASIC_EC", "ECNAIVE"}));
...
private:
    ...
    struct SaveEncodeScopeTimer {
        BasicECNative* owner;
        std::chrono::steady_clock::time_point t0;
        explicit SaveEncodeScopeTimer(BasicECNative* o)
    ...
    struct SaveNetScopeTimer {
        BasicECNative* owner;
        std::chrono::steady_clock::time_point t0;
        explicit SaveNetScopeTimer(BasicECNative* o)
            : owner(o), t0(std::chrono::steady_clock::now()) {}
        ~SaveNetScopeTimer() {
            if (owner != nullptr && !owner->is_load_mode_) {
                const auto t1 = std::chrono::steady_clock::now();
    ...
    struct LoadNetScopeTimer {
        BasicECNative* owner;
        std::chrono::steady_clock::time_point t0;
        explicit LoadNetScopeTimer(BasicECNative* o)
            : owner(o), t0(std::chrono::steady_clock::now()) {}
        ~LoadNetScopeTimer() {
            if (owner != nullptr && owner->is_load_mode_) {
                const auto t1 = std::chrono::steady_clock::now();
    ...
    struct LoadRecvNetScopeTimer {
        BasicECNative* owner;
        std::chrono::steady_clock::time_point t0;
        explicit LoadRecvNetScopeTimer(BasicECNative* o)
            : owner(o), t0(std::chrono::steady_clock::now()) {}
    ...
    void init_ec_encoding() {
        ...
        if (a_mat_ == nullptr) {
            ...
            throw std::runtime_error("BasicEC: Failed to allocate EC encoding matrix");
        }
        ...
        if (g_tbls_ == nullptr) {
            std::cerr << "BasicEC: Failed to allocate EC encoding tables" << std::endl;
            free(a_mat_);
            a_mat_ = nullptr;
            throw std::runtime_error("BasicEC: Failed to allocate EC encoding tables");
        }
    ...
    void encode_ec_blocks(const std::vector<uintptr_t>& data_addrs,
        ...
        if (g_tbls_ == nullptr || a_mat_ == nullptr) {
            std::cerr << "BasicEC: ERROR: EC encoding tables not initialized!" << std::endl;
            throw std::runtime_error("BasicEC: EC encoding tables not initialized");
        }
        if (static_cast<int>(data_addrs.size()) != k_) {
            throw std::runtime_error("BasicEC: data_addrs size must equal k_");
        }
    ...
    void init_rdma_resources() {
        ...
        if (ibv_fork_init() != 0) {
            std::cerr << "[BasicEC RDMA] WARNING: ibv_fork_init() failed. Forked processes may get Bad address."
                      << std::endl;
        }
    ...
    void init_rdma_software_load_channel() {
        ...
        try {
            if (rank_ == 2) {
                int sock = conn_.get_basic_ec_load_recv_rank3_data1_socket().native_handle();
                ...
                int sock = conn_.get_basic_ec_load_send_rank3_data1_socket().native_handle();
                rdma_software_load_channel_ = std::make_unique<RdmaConnectionChannel>(
                    rdma_context_, rdma_pd_, rdma_software_load_send_cq_, rdma_software_load_recv_cq_,
            ...
            std::cerr << "[BasicEC RDMA] Failed to init software-only load channel: " << e.what() << std::endl;
    ...
    void start_threads() {
        ec_rs_pool_init();
        for (int i = 0; i < num_channels_; ++i) {
            send_threads_[i] = std::thread(&BasicECNative::send_worker, this, i);
            recv_threads_[i] = std::thread(&BasicECNative::recv_worker, this, i);
        }
    }
    ...
    void init_rdma_exchange_sockets() {
        ...
        std::thread accept_thread([this]() {
            auto do_listen_accept = [this](const std::string& ip, uint16_t port, int& out_fd, const std::string& name) {
                ...
                if (listen_fd < 0) {
                    throw std::runtime_error(std::string("[BasicEC RDMA] socket() failed for ") + name + ": " + std::strerror(errno));
                }
                int opt = 1;
                ...
                if (inet_pton(AF_INET, ip.c_str(), &addr.sin_addr) <= 0) {
                    close(listen_fd);
                    throw std::runtime_error(std::string("[BasicEC RDMA] inet_pton failed for ") + name);
                }
                if (bind(listen_fd, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
                    close(listen_fd);
                    throw std::runtime_error(std::string("[BasicEC RDMA] bind failed for ") + name + ": " + std::strerror(errno));
                }
                if (listen(listen_fd, 1) < 0) {
                    close(listen_fd);
                    throw std::runtime_error(std::string("[BasicEC RDMA] listen failed for ") + name + ": " + std::strerror(errno));
                }
                ...
                close(listen_fd);
                if (out_fd < 0) {
                    throw std::runtime_error(std::string("[BasicEC RDMA] accept failed for ") + name + ": " + std::strerror(errno));
                }
            };
            for (int i = 0; i < num_channels_; ++i) {
        ...
        auto do_connect = [this](const std::string& ip, uint16_t port, int& out_fd, const std::string& name) {
            ...
            for (int attempt = 0; attempt < max_retries; ++attempt) {
                ...
                if (attempt == max_retries - 1) {
                    close(out_fd);
                    out_fd = -1;
                    throw std::runtime_error(std::string("[BasicEC RDMA] connect failed for ") + name + ": " + std::strerror(errno));
                }
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
            }
        };
    ...
    void init_rdma_save_channels() {
        ...
        try {
            ...
            std::cerr << "[BasicEC RDMA] Save channel init failed: " << e.what()
    ...
    void recv_parity1_worker() {
        while (!stop_) {
            ...
            if (!conn_.is_recv_parity1_connected()) {
                std::cerr << "BasicEC: ERROR: recv_parity1 socket not connected!" << std::endl;
                throw std::runtime_error("BasicEC: recv_parity1 socket not connected");
            }
            ...
            if (use_rdma_ && recv_parity1_channel_) {
                ...
                if (!recv_with_size_bool(
                        ...
                        task.size)) {
                    std::cerr << "BasicEC: recv_parity1_with_size_bool returned false" << std::endl;
                    throw std::runtime_error("BasicEC: recv_parity1_with_size_bool returned false");
                }
            }
    ...
    void recv_parity0_worker() {
        while (!stop_) {
            ...
            if (!conn_.is_recv_parity0_connected()) {
                std::cerr << "BasicEC: ERROR: recv_parity0 socket not connected!" << std::endl;
                throw std::runtime_error("BasicEC: recv_parity0 socket not connected");
            }
            ...
            if (use_rdma_ && recv_parity0_channel_) {
                ...
                if (!recv_with_size_bool(
                        ...
                        reinterpret_cast<void*>(task.addr),
                        task.size)) {
                    std::cerr << "BasicEC: recv_parity0_with_size_bool returned false" << std::endl;
                    throw std::runtime_error("BasicEC: recv_parity0_with_size_bool returned false");
                }
            }
    ...
    void recv_data1_worker() {
        while (!stop_) {
            ...
            if (!conn_.is_recv_data1_connected()) {
                std::cerr << "BasicEC: ERROR: recv_data1 socket not connected!" << std::endl;
                throw std::runtime_error("BasicEC: recv_data1 socket not connected");
            }
            ...
            if (use_rdma_ && recv_data1_channel_) {
                ...
                if (!recv_with_size_bool(
                        ...
                        reinterpret_cast<void*>(task.addr),
                        task.size)) {
                    std::cerr << "BasicEC: recv_data1_with_size_bool returned false" << std::endl;
                    throw std::runtime_error("BasicEC: recv_data1_with_size_bool returned false");
                }
            }
        ...
        while (!stop_) {
            ...
            try {
                ...
                if (!recv_with_size_bool(conn_.recv_socket(idx),
                    ...
                    std::cerr << "BasicEC: RecvWorker[" << idx << "] recv failed" << std::endl;
                }
            }
...
public:
    ...
    void submit_basic_ec_save_general(
    ...
    static std::array<int, kEcRsPoolSize> parse_xor_pool_cpus_or_throw() {
        ...
        while (*p) {
            ...
            if (end == p || v < 0 || v > 65535) {
                throw std::runtime_error("BASIC_EC_RS_CPU_LIST: invalid CPU id token");
            }
        ...
        if (parsed.size() != static_cast<size_t>(kEcRsPoolSize)) {
            throw std::runtime_error(
                "BASIC_EC_RS_CPU_LIST must contain exactly 16 comma-separated CPU ids "
    ...
    void ec_rs_pool_init() {
        ...
        for (int i = 0; i < kEcRsPoolSize; ++i) {
            ...
            int rc = pthread_create(
                ...
                nullptr,
                &BasicECNative::ec_rs_pool_pthread_entry,
            ...
            if (rc != 0) {
                ...
                throw std::runtime_error("BasicEC: pthread_create for EC RS pool failed: " +
...
PYBIND11_MODULE(basic_ec_native, m) {
    pybind11::class_<BasicECNative>(m, "BasicECNative")
        ...
        .def("register_buffer", &BasicECNative::register_buffer,
             "Register buffer for RDMA operations",
             pybind11::arg("addr"),
             pybind11::arg("size"))
        .def("unregister_buffer", &BasicECNative::unregister_buffer,
             "Unregister buffer from RDMA",
             pybind11::arg("addr"))
        // Generalized save mode submit function (k+2 scheme)
        .def("submit_basic_ec_save_general", &BasicECNative::submit_basic_ec_save_general,
        ...
        .def("submit_basic_ec_save", &BasicECNative::submit_basic_ec_save,
        ...
        .def("submit_send_sentinels", &BasicECNative::submit_send_sentinels,
             pybind11::arg("num_sends"))
        .def("submit_recv_sentinels", &BasicECNative::submit_recv_sentinels,
             pybind11::arg("num_recvs"))
        // Generalized single-task submit for recovery
        .def("submit_send_task", &BasicECNative::submit_send_task,
             pybind11::arg("channel_idx"),
             pybind11::arg("addr"),
             pybind11::arg("size"))
        .def("submit_recv_task", &BasicECNative::submit_recv_task,
             pybind11::arg("channel_idx"),
             pybind11::arg("addr"),
             pybind11::arg("size"))
        // Legacy submit functions
        .def("submit_send_data1", &BasicECNative::submit_send_data1,
             pybind11::arg("send_addr"), pybind11::arg("size"))
        .def("submit_send_parity0", &BasicECNative::submit_send_parity0,
             pybind11::arg("send_addr"), pybind11::arg("size"))
        .def("submit_send_parity1", &BasicECNative::submit_send_parity1,
             pybind11::arg("send_addr"), pybind11::arg("size"))
        .def("submit_recv_parity1", &BasicECNative::submit_recv_parity1,
             pybind11::arg("recv_addr"), pybind11::arg("size"))
        .def("submit_recv_parity0", &BasicECNative::submit_recv_parity0,
             pybind11::arg("recv_addr"), pybind11::arg("size"))
        .def("submit_recv_data1", &BasicECNative::submit_recv_data1,
             pybind11::arg("recv_addr"), pybind11::arg("size"))
        // Common functions
        .def("get_data_buffers_to_release", &BasicECNative::get_data_buffers_to_release)
        .def("get_parity_buffers_to_release", &BasicECNative::get_parity_buffers_to_release)
        .def("reset_encoding_completion_flags", &BasicECNative::reset_encoding_completion_flags)
        .def("get_ft_timing_stats", &BasicECNative::get_ft_timing_stats,
             "Return per-rank timing: net_s wall-span; encode_s serial-equivalent encode CPU sum")
        .def("reset_load_timing_stats", &BasicECNative::reset_load_timing_stats,
             "Reset software/load timing counters without rebuilding connections")
        .def("wait_for_encoding_completion", &BasicECNative::wait_for_encoding_completion)
        .def("wait_for_pending_network_tasks", &BasicECNative::wait_for_pending_network_tasks,
             "Wait until all in-flight save-path send/recv tasks complete (no sentinels)")
        // Legacy save mode sentinels
        .def("submit_send_data1_sentinel", &BasicECNative::submit_send_data1_sentinel)
        .def("submit_send_parity0_sentinel", &BasicECNative::submit_send_parity0_sentinel)
        .def("submit_send_parity1_sentinel", &BasicECNative::submit_send_parity1_sentinel)
        .def("submit_recv_parity1_sentinel", &BasicECNative::submit_recv_parity1_sentinel)
        .def("submit_recv_parity0_sentinel", &BasicECNative::submit_recv_parity0_sentinel)
        .def("submit_recv_data1_sentinel", &BasicECNative::submit_recv_data1_sentinel)
        // Load mode functions
        .def("set_load_mode", &BasicECNative::set_load_mode,
        ...
        .def("software_send_rank3_data1", &BasicECNative::software_send_rank3_data1,
             "Software failure mode: rank3 send d21 to rank2",
             pybind11::arg("send_addr"),
             pybind11::arg("size"))
        .def("software_recv_data1", &BasicECNative::software_recv_data1,
        ...
        .def("init_basic_ec_load_sw_bind_listen",
             &BasicECNative::init_basic_ec_load_sw_bind_listen,
        ...
        .def("submit_basic_ec_decode_recovery", &BasicECNative::submit_basic_ec_decode_recovery,
