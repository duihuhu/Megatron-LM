#!/bin/bash
# Clean build script for BasicEC native C++ module
...
echo "=========================================="
echo "Building BasicEC Native C++ Module"
...
if [ $ISAL_FOUND -eq 0 ]; then
    echo "❌ Error: ISA-L library not found. BasicEC requires ISA-L for erasure coding."
...
rm -f basic_ec_native*.so ecnaive_native*.so
rm -f basic_ec_native*.pyd ecnaive_native*.pyd
rm -rf build/
rm -rf *.egg-info/
...
cp setup_simple_basic_ec.py "$TEMP_DIR/"
cp basic_ec_native.cpp "$TEMP_DIR/"
...
python3 setup_simple_basic_ec.py build_ext --inplace

# Copy built module back
cd "$SCRIPT_DIR"
cp "$TEMP_DIR"/basic_ec_native*.so . 2>/dev/null || true
cp "$TEMP_DIR"/basic_ec_native*.pyd . 2>/dev/null || true

# Clean up
rm -rf "$TEMP_DIR"

# Check if build was successful
if [ -f basic_ec_native*.so ] || [ -f basic_ec_native*.pyd ]; then
    echo ""
    echo "=========================================="
    echo "✅ BasicEC native module built successfully!"
    echo "=========================================="
    echo "Module location: $(pwd)/basic_ec_native*.so"
    echo ""
    ...
    echo ""
    echo "Usage:"
    echo "  Basic (ASIO/TCP):  --use-basic-ec"
    if [ $RDMA_FOUND -eq 1 ]; then
        echo "  With RDMA:         --use-basic-ec --use-rdma"
    else
        echo "  With RDMA:         (Install RDMA libraries first)"
    fi
    echo ""
    echo "To test the module, run:"
    echo "  python3 -c 'import basic_ec_native; print(\"BasicEC module loaded successfully\")'"
else
    echo ""
    echo "=========================================="
    echo "❌ Build failed!"
