#!/bin/bash
# Clean build script for CheckCode native C++ module
# Renamed from build_clean_eclatin.sh; still cleans legacy eclatin_native*.so if present.
...
#
# Usage:
#   bash build_clean_checkcode.sh          # Build without CUDA (PyTorch mode)
#   bash build_clean_checkcode.sh cuda     # Build with CUDA support
...
echo "=========================================="
echo "Building CheckCode Native C++ Module"
...
rm -f checkcode_native*.so
rm -f checkcode_native*.pyd
...
cp setup_simple_checkcode.py "$TEMP_DIR/"
cp checkcode_native.cpp "$TEMP_DIR/"
...
python3 setup_simple_checkcode.py build_ext --inplace

# Copy built module back
cd "$SCRIPT_DIR"
cp "$TEMP_DIR"/checkcode_native*.so . 2>/dev/null || true
cp "$TEMP_DIR"/checkcode_native*.pyd . 2>/dev/null || true

# Clean up
rm -rf "$TEMP_DIR"

# Check if build was successful
if [ -f checkcode_native*.so ] || [ -f checkcode_native*.pyd ]; then
    echo ""
    echo "=========================================="
    echo "✅ CheckCode native module built successfully!"
    echo "=========================================="
    echo "Module location: $(pwd)/checkcode_native*.so"
    echo ""
    ...
    if [ "$USE_CUDA" == "true" ]; then
        ...
        echo "  - To enable: bash build_clean_checkcode.sh cuda"
    fi
    echo ""
    echo "Usage:"
    echo "  Basic (ASIO/TCP):  --use-checkcode"
    echo "  With layerwise:    --use-checkcode --use-checkcode-layerwise"
    if [ $RDMA_FOUND -eq 1 ]; then
        echo "  With RDMA:         --use-checkcode --use-rdma"
        echo "  RDMA + layerwise:  --use-checkcode --use-checkcode-layerwise --use-rdma"
    else
        echo "  With RDMA:         (Install RDMA libraries first)"
    fi
    echo ""
    echo "To test the module, run:"
    echo "  python3 -c 'import checkcode_native; print(\"RDMA available:\", checkcode_native.is_rdma_available())'"
else
    echo ""
    echo "=========================================="
    echo "❌ Build failed!"
