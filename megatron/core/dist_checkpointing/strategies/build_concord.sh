#!/bin/bash
# Clean build script for Concord native C++ module (RDMA only, no ASIO fallback)
# Compiles concord_native.cpp → concord_native*.so
...
echo "=========================================="
echo "Building Concord Native C++ Module"
...
rm -f concord_native.cpython-*.so
rm -f concord_native*.so
rm -rf build/
rm -rf *.egg-info/
...
cp setup_simple_concord.py "$TEMP_DIR/"
cp concord_native.cpp "$TEMP_DIR/"
cp rdma_device_utils.h "$TEMP_DIR/"

cd "$TEMP_DIR"

python3 -c "$PY_FILTER_PATH sys.argv=['build_concord','build_ext','--inplace']; exec(open('setup_simple_concord.py').read())"

# Copy built module back
cd "$SCRIPT_DIR"
cp "$TEMP_DIR"/concord_native.cpython-*.so . 2>/dev/null || true

# Clean up
rm -rf "$TEMP_DIR"

# Verify
if ls concord_native.cpython-*.so 1>/dev/null 2>&1; then
    SO_FILE=$(ls concord_native.cpython-*.so | head -1)
    echo ""
    echo "=========================================="
    echo "✅ Concord native module built successfully!"
    echo "=========================================="
    ...
    python3 -c "$PY_FILTER_PATH import importlib.util; spec=importlib.util.spec_from_file_location('concord_native', '$SCRIPT_DIR/$SO_FILE'); mod=importlib.util.module_from_spec(spec); spec.loader.exec_module(mod); n=mod.ConcordNative(4); n.compile_plans(0); print(f'  n={n.n()} num_stripes={n.num_stripes()}'); print(f'  GDR available: {mod.ConcordNative.gdr_available()}'); print('  POA auto-generation: OK')" && echo "  Smoke test passed!" || echo "  ⚠ Smoke test failed"

    echo ""
    echo "Usage:"
    echo "  Training:  --use-concord --concord-n 4 --concord-table-dir <dir>"
