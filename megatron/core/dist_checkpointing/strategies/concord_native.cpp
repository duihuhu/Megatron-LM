...
struct RsPoolWorkerCtx {
    class ConcordNative* self = nullptr;
    int wid = 0;
};
...
static inline uint64_t concord_now_us() {
...
// ---------------------------------------------------------------------------
static constexpr size_t CONCORD_DEFAULT_RDMA_CHUNK = 64ULL * 1024 * 1024;

static size_t concord_rdma_chunk_size() {
    static const size_t chunk = []() -> size_t {
        const char* env = std::getenv("CONCORD_RDMA_CHUNK_MB");
        if (!env || !*env) return CONCORD_DEFAULT_RDMA_CHUNK;
        char* end = nullptr;
        errno = 0;
        unsigned long long value = std::strtoull(env, &end, 10);
        if (errno != 0 || end == env || *end != '\0' || value == 0 || value > 4095)
            throw std::runtime_error("Concord RDMA: invalid CONCORD_RDMA_CHUNK_MB");
        return static_cast<size_t>(value) * 1024ULL * 1024ULL;
    }();
    return chunk;
}
static constexpr int    CONCORD_MAX_WR = 64;
static constexpr int    CONCORD_DEFAULT_MAX_SEND_SGE = 16;
...
class ConcordRdmaChannel {
public:
    // Per-channel design: each channel owns dedicated send/recv CQs.
    ConcordRdmaChannel(ibv_context* ctx, ibv_pd* pd,
        ...
        : ctx_(ctx), pd_(pd),
          ...
          segment_chunk_size_(concord_rdma_chunk_size()), connected_(false)
    {
        ...
        if (!send_cq_ || !recv_cq_) {
            ...
            throw std::runtime_error("Concord RDMA: failed to create per-channel CQs");
        }
        ...
        attr.cap.max_send_wr = CONCORD_MAX_WR;
        attr.cap.max_recv_wr = CONCORD_MAX_WR;
        int requested_max_send_sge = CONCORD_DEFAULT_MAX_SEND_SGE;
        if (const char* env = std::getenv("CONCORD_MAX_SEND_SGE")) {
            try {
                requested_max_send_sge = std::max(1, std::stoi(env));
            } catch (...) {
                throw std::runtime_error("Concord RDMA: invalid CONCORD_MAX_SEND_SGE");
            }
        }
        ...
        if (!qp_)
            throw std::runtime_error("Concord RDMA: failed to create QP");
    ...
    void exchange_and_connect() {
        ...
        // Each endpoint writes its fixed-size QP record before reading the peer record.
        ssize_t ns = send(tcp_sock_, &local, sizeof(local), 0);
        if (ns != (ssize_t)sizeof(local))
            throw std::runtime_error("Concord RDMA: failed to send conn info");
        ssize_t nr = recv(tcp_sock_, &remote, sizeof(remote), MSG_WAITALL);
        if (nr != (ssize_t)sizeof(remote))
            throw std::runtime_error("Concord RDMA: failed to recv conn info");

        connect_qp(remote);

        uint64_t local_segment_caps[2] = {
            htobe64(static_cast<uint64_t>(std::min(max_send_sge_, max_recv_sge_))),
            htobe64(static_cast<uint64_t>(concord_rdma_chunk_size())),
        };
        ...
        if (send(tcp_sock_, local_segment_caps, sizeof(local_segment_caps), 0) !=
            ...
            throw std::runtime_error("Concord RDMA: failed to send segment capabilities");
        if (recv(tcp_sock_, remote_segment_caps, sizeof(remote_segment_caps), MSG_WAITALL) !=
            ...
            throw std::runtime_error("Concord RDMA: failed to recv segment capabilities");
        ...
        if (remote_sge == 0 || remote_chunk == 0)
            throw std::runtime_error("Concord RDMA: invalid peer segment capabilities");
        ...
        segment_chunk_size_ = std::min(
            concord_rdma_chunk_size(), static_cast<size_t>(remote_chunk));
    }
    ...
    void send_data(const uint8_t* data, size_t size,
        ...
        if (!connected_)
            throw std::runtime_error("Concord RDMA: channel not connected");

        if (wait_cb) wait_cb();
        // Send size over TCP
        uint64_t net_sz = htobe64(size);
        if (send(tcp_sock_, &net_sz, sizeof(net_sz), 0) != sizeof(net_sz))
            throw std::runtime_error("Concord RDMA: failed to send size");
        uint8_t ack;
        if (recv(tcp_sock_, &ack, 1, MSG_WAITALL) != 1)
            throw std::runtime_error("Concord RDMA: failed to recv ack");
        if (done_cb) done_cb();

        ibv_mr* mr = find_mr((uintptr_t)data, size);
        if (!mr) {
            throw std::runtime_error(
                "Concord RDMA: unregistered send buffer (addr=0x" +
    ...
    void send_segments(const std::vector<std::pair<uintptr_t, size_t>>& segments,
        ...
        if (segment_total != logical_size)
            throw std::runtime_error("Concord RDMA: scatter logical size mismatch");
        if (wait_cb) wait_cb();
        uint64_t net_sz = htobe64(logical_size);
        if (send(tcp_sock_, &net_sz, sizeof(net_sz), 0) != sizeof(net_sz))
            throw std::runtime_error("Concord RDMA: failed to send scatter size");
        uint8_t ack;
        if (recv(tcp_sock_, &ack, 1, MSG_WAITALL) != 1)
            throw std::runtime_error("Concord RDMA: failed to recv scatter ack");
        if (done_cb) done_cb();
    ...
    size_t recv_segments(
        ...
        if (recv(tcp_sock_, &net_sz, sizeof(net_sz), MSG_WAITALL) != sizeof(net_sz))
            throw std::runtime_error("Concord RDMA: failed to recv scatter size");
        const size_t size = be64toh(net_sz);
        if (size != logical_size)
            throw std::runtime_error("Concord RDMA: scatter receive size mismatch");
        uint8_t ack = 1;
        if (send(tcp_sock_, &ack, 1, 0) != 1)
            throw std::runtime_error("Concord RDMA: failed to send scatter ack");
        if (done_cb) done_cb();
    ...
    size_t recv_data(uint8_t* buf, size_t buf_size,
        ...
        uint64_t net_sz;
        if (recv(tcp_sock_, &net_sz, sizeof(net_sz), MSG_WAITALL) != sizeof(net_sz))
            throw std::runtime_error("Concord RDMA: failed to recv size");
        size_t size = be64toh(net_sz);
        if (size > buf_size)
            throw std::runtime_error("Concord RDMA: recv size " + std::to_string(size) +
                " exceeds buffer " + std::to_string(buf_size) +
                " (delta=" + std::to_string(size - buf_size) + ")");

        uint8_t ack = 1;
        if (send(tcp_sock_, &ack, 1, 0) != 1)
            throw std::runtime_error("Concord RDMA: failed to send ack");
        if (done_cb) done_cb();

        ibv_mr* mr = find_mr((uintptr_t)buf, size);
        if (!mr) {
            throw std::runtime_error(
                "Concord RDMA: unregistered recv buffer (addr=0x" +
...
private:
    ...
    void connect_qp(const RdmaConnInfo& remote) {
        ...
        if (ibv_modify_qp(qp_, &attr,
            ...
            throw std::runtime_error("Concord RDMA: INIT failed");

        ibv_port_attr pa;
        if (ibv_query_port(ctx_, 1, &pa) != 0)
            throw std::runtime_error("Concord RDMA: query port failed");
        ...
        if (ibv_modify_qp(qp_, &attr,
            ...
            throw std::runtime_error("Concord RDMA: RTR failed");
        ...
        if (ibv_modify_qp(qp_, &attr,
            ...
            throw std::runtime_error("Concord RDMA: RTS failed");
        connected_ = true;
    }
        ...
        while (remaining > 0) {
            ...
            while (message_remaining > 0 &&
                ...
                if (segment_idx >= segments.size())
                    throw std::runtime_error("Concord RDMA: scatter segments ended early");
                ...
                if (!mr)
                    throw std::runtime_error(
                        "Concord RDMA: unregistered scatter segment addr=" +
            ...
            if (ibv_post_send(qp_, &wr, &bad))
                throw std::runtime_error("Concord RDMA: scatter post_send failed");
            poll_cq(send_cq_, 1);
            if (done_cb) done_cb();
        }
    }
    ...
    void validate_segments_(
        ...
        for (const auto& segment : segments) {
            if (segment.first == 0 || segment.second == 0)
                throw std::runtime_error(std::string("Concord RDMA: empty ") + operation + " segment");
            if (segment.second > std::numeric_limits<size_t>::max() - total)
                throw std::runtime_error("Concord RDMA: segment size overflow");
            total += segment.second;
            if (!find_mr(segment.first, segment.second))
                throw std::runtime_error(
                    std::string("Concord RDMA: unregistered ") + operation +
                    " segment addr=" + std::to_string(segment.first) +
                    " size=" + std::to_string(segment.second));
        }
        if (segments.empty() || total != logical_size)
            throw std::runtime_error(std::string("Concord RDMA: ") + operation +
    ...
    void recv_segments_chunked_(
        ...
        while (remaining > 0) {
            ...
            while (message_remaining > 0 &&
                ...
                if (segment_idx >= segments.size())
                    throw std::runtime_error("Concord RDMA: receive segments ended early");
                const auto& segment = segments[segment_idx];
                const size_t length = std::min(message_remaining,
                                               segment.second - segment_offset);
                ibv_mr* mr = find_mr(segment.first + segment_offset, length);
                if (!mr)
                    throw std::runtime_error("Concord RDMA: receive segment MR disappeared");
                ibv_sge sge{};
                sge.addr = static_cast<uint64_t>(segment.first + segment_offset);
                sge.length = static_cast<uint32_t>(length);
                sge.lkey = mr->lkey;
            ...
            if (ibv_post_recv(qp_, &wr, &bad))
                throw std::runtime_error("Concord RDMA: scatter post_recv failed");
            poll_cq(recv_cq_, 1);
            if (done_cb) done_cb();
        }
    }
    ...
    void _send_chunked(const uint8_t* data, size_t total, ibv_mr* mr,
        ...
        while (remaining > 0) {
            if (wait_cb) wait_cb();
            size_t nchunks = (std::min(remaining, concord_rdma_chunk_size()) + concord_rdma_chunk_size() - 1) / concord_rdma_chunk_size();
            std::vector<ibv_sge> sge(nchunks);
            std::vector<ibv_send_wr> wr(nchunks);
            for (size_t i = 0; i < nchunks; ++i) {
                size_t cur = std::min(concord_rdma_chunk_size(), remaining);
            ...
            if (ibv_post_send(qp_, &wr[0], &bad))
                throw std::runtime_error("Concord RDMA: post_send failed");
    ...
    int post_recv_chunked_(uint8_t* buf, size_t total, ibv_mr* mr) {
        ...
        int posted = 0;
        while (remaining > 0) {
            size_t group_bytes = std::min(remaining, concord_rdma_chunk_size());
            size_t nchunks = (group_bytes + concord_rdma_chunk_size() - 1) /
                             concord_rdma_chunk_size();
            std::vector<ibv_sge> sge(nchunks);
            ...
            if (ibv_post_recv(qp_, &wr[0], &bad))
                throw std::runtime_error("Concord RDMA: prepost recv failed");
    ...
    void _recv_chunked(uint8_t* buf, size_t total, ibv_mr* mr,
        ...
        while (remaining > 0) {
            ...
            ibv_recv_wr* bad = nullptr;
            if (ibv_post_recv(qp_, &wr[0], &bad))
                throw std::runtime_error("Concord RDMA: post_recv failed");
    ...
    void poll_cq(ibv_cq* cq, int count) {
        ...
        while (done < count) {
            ...
            if (n < 0) throw std::runtime_error("Concord RDMA: poll CQ error");
            if (n > 0) {
                if (wc.status != IBV_WC_SUCCESS)
                    throw std::runtime_error(
                        "Concord RDMA: CQ error status=" + std::to_string(wc.status));
                ++done;
            }
        }
    }
...
public:
    void start_tag_receiver() {
        ...
        tag_receiver_thread_ = std::thread(&ConcordRdmaChannel::tag_receiver_loop_, this);
    }
    ...
    void send_tagged(uint64_t tag, const uint8_t* data, size_t size,
        ...
        if (!write_full_locked_(&c, sizeof(c)))
            throw std::runtime_error("Concord RDMA: failed to send tagged header");
        const uint64_t ack_wait_t0 = concord_now_us();
        {
        ...
        const uint64_t ack_wait_us = concord_now_us() - ack_wait_t0;
        ...
        ibv_mr* mr = find_mr((uintptr_t)data, size);
        if (!mr)
            throw std::runtime_error("Concord RDMA: unregistered tagged send buffer");
        send_chunked(data, size, mr, wait_cb, done_cb);
    }
    ...
    void send_tagged_segments(
        ...
        for (const auto& segment : segments) segment_total += segment.second;
        if (segment_total != logical_size)
            throw std::runtime_error("Concord RDMA: tagged scatter logical size mismatch");
        if (wait_cb) wait_cb();
        {
            std::lock_guard<std::mutex> lk(tag_mtx_);
            ack_ready_[tag] = false;
        ...
        if (!write_full_locked_(&c, sizeof(c)))
            throw std::runtime_error("Concord RDMA: failed to send tagged scatter header");
        const uint64_t ack_wait_t0 = concord_now_us();
        {
            std::unique_lock<std::mutex> lk(tag_mtx_);
            tag_cv_.wait(lk, [&]{
    ...
    void prepare_tagged_recv(uint64_t tag, uint8_t* buf, size_t capacity) {
        if (!buf || capacity == 0)
            throw std::runtime_error("Concord prepared tagged receive: empty target");
        if (!find_mr(reinterpret_cast<uintptr_t>(buf), capacity))
            throw std::runtime_error("Concord prepared tagged receive: unregistered target");
        std::lock_guard<std::mutex> lk(tag_mtx_);
        if (prepared_tagged_recvs_.count(tag) || pending_data_sizes_.count(tag))
            throw std::runtime_error("Concord prepared tagged receive: duplicate or late tag");
    ...
    size_t wait_prepared_tagged_recv(uint64_t tag) {
        ...
        std::string error;
        {
            ...
            if (it == prepared_tagged_recvs_.end())
                throw std::runtime_error("Concord prepared tagged receive: unknown tag");
            completions = it->second.completions;
            size = it->second.size;
            error = it->second.error;
        }
        if (!error.empty())
            throw std::runtime_error("Concord prepared tagged receive: " + error);
        if (tag_stop_.load(std::memory_order_acquire) && completions == 0)
            throw std::runtime_error("Concord prepared tagged receive: channel stopped");
        {
            std::lock_guard<std::mutex> recv_lock(recv_mtx_);
    ...
    size_t recv_tagged(uint64_t tag, uint8_t* buf, size_t buf_size,
        ...
        if (size > buf_size) {
            throw std::runtime_error("Concord tagged recv: size " + std::to_string(size) +
                                     " exceeds buffer " + std::to_string(buf_size) +
                                     " (tag=" + std::to_string(tag) + ")");
        }
        ibv_mr* mr = find_mr((uintptr_t)buf, size);
        if (!mr) {
            throw std::runtime_error("Concord RDMA: unregistered tagged recv buffer");
        }
        std::lock_guard<std::mutex> lock(recv_mtx_);
        TaggedCtl ack{htobe32(kTagAck), 0, htobe64(tag), 0};
        if (!write_full_locked_(&ack, sizeof(ack))) {
            throw std::runtime_error("Concord RDMA: failed to send tagged ack");
        }
        recv_chunked(buf, size, mr, wait_cb, done_cb);
        return size;
    }
    ...
    size_t recv_tagged_segments(
        ...
        if (size != logical_size)
            throw std::runtime_error("Concord tagged scatter receive size mismatch");
        std::lock_guard<std::mutex> lock(recv_mtx_);
        TaggedCtl ack{htobe32(kTagAck), 0, htobe64(tag), 0};
        if (!write_full_locked_(&ack, sizeof(ack)))
            throw std::runtime_error("Concord RDMA: failed to send tagged scatter ack");
        recv_segments_chunked_(segments, logical_size, wait_cb, done_cb);
        return size;
    }
...
public:
          ...
          acceptor_fd_(-1), acceptor_thread_stop_(false)
    {
        if (n == 2) {
            ...
            int lanes = 1;
            const char* send_lanes_env = std::getenv("CONCORD_SEND_LANES_PER_PEER");
            const char* recv_lanes_env = std::getenv("CONCORD_RECV_LANES_PER_PEER");
            if (send_lanes_env && send_lanes_env[0] != '\0' &&
                ...
                const char* lanes_env = std::getenv("CONCORD_RDMA_LANES_PER_PEER");
    ...
    {
        if (!use_rdma) {
            throw std::runtime_error("Concord: RDMA is required (no ASIO fallback)");
        }
        if (group_size != n_) {
            throw std::runtime_error("Concord: group_size must equal POA n");
        }
        ...
        if (debug_)
            std::cout << "[Concord RDMA] rank_in_group=" << rank_in_group
                      << " listening on port " << listen_port << std::endl;

        acceptor_fd_ = socket(AF_INET, SOCK_STREAM, 0);
        if (acceptor_fd_ < 0)
            throw std::runtime_error("Concord RDMA: failed to create acceptor socket");
        int reuse = 1;
        ...
        if (inet_pton(AF_INET, my_ip.c_str(), &listen_addr.sin_addr) <= 0)
            throw std::runtime_error("Concord RDMA: invalid listen IP: " + my_ip);
        if (bind(acceptor_fd_, (struct sockaddr*)&listen_addr, sizeof(listen_addr)) < 0)
            throw std::runtime_error("Concord RDMA: bind failed on port " + std::to_string(listen_port));
        if (listen(acceptor_fd_, std::max(group_size * 16, 128)) < 0)
            throw std::runtime_error("Concord RDMA: listen failed");

        num_stripes_ = (int)table_.size();
        if (num_stripes_ <= 0)
            throw std::runtime_error("Concord RDMA: POA table has no stripes");
                ...
                if (requested_lanes > 0) {
                    ...
                    if (bounded_lanes < num_stripes_) {
                        ...
                        // protocol (per-channel reader thread + DATA/ACK records).
                                                const char* unsafe_env = std::getenv("CONCORD_ALLOW_UNSAFE_LANE_SHARING");
                        if (!unsafe_env || std::atoi(unsafe_env) == 0) {
                            throw std::runtime_error(
                                "Concord RDMA: CONCORD_RDMA_LANES_PER_PEER < num_stripes uses the "
                                "experimental tagged shared-lane path; set "
                                "CONCORD_ALLOW_UNSAFE_LANE_SHARING=1 to enable it");
                        }
                    }
        ...
        if (debug_)
            std::cout << "[Concord RDMA] rank=" << rank_in_group
        ...
        // channels_ owns one channel pointer per peer and physical lane.
        channels_.assign(group_size, std::vector<ConcordRdmaChannel*>(num_lanes_, nullptr));
        ...
        auto connect_outbound = [&](int peer, int lane) {
            ...
            if (send(sock, &hello, sizeof(hello), 0) != (ssize_t)sizeof(hello)) {
                close(sock);
                throw std::runtime_error(
                    "Concord RDMA: failed to send hello to peer=" + std::to_string(peer) +
                    " lane=" + std::to_string(lane));
            }
            auto ch = std::make_unique<ConcordRdmaChannel>(
                rdma_ctx_, rdma_pd_,
                sock, peer, &registered_bufs_, &buf_mtx_);
            ch->exchange_and_connect();
            channels_[peer][lane] = ch.release();
            channel_owners_.push_back(
                std::unique_ptr<ConcordRdmaChannel>(channels_[peer][lane]));
        };
            ...
            for (int lane = 0; lane < num_lanes_; ++lane) {
                int sock;
                {
                    ...
                    if (acceptor_thread_stop_)
                        throw std::runtime_error("Concord: acceptor thread stopped prematurely");
        ...
        // The native runtime owns one shared RS encode worker pool.
        rs_pool_init();

        // Pre-compile stripe plans only for real Concord POA mode.
        if (n_ >= 3) {
    ...
    void clear_recovery_buffers() {
        ...
        if (pending_chunks != 0) {
            throw std::runtime_error(
                "Concord: clear_recovery_buffers called with pending recovery chunks: " +
        ...
        size_t block_size)
    {
        ...
        if (survivor_positions.size() != static_cast<size_t>(k) ||
            ...
            throw std::runtime_error(
                "Concord decode: survivor count/address mismatch: positions=" +
        ...
        for (uintptr_t addr : survivor_addrs) {
            if (addr == 0)
                throw std::runtime_error("Concord decode: null survivor address");
            if (!unique_survivor_addrs.insert(addr).second)
                throw std::runtime_error("Concord decode: duplicate survivor address");
        }
        std::set<uintptr_t> unique_outputs;
        for (uintptr_t addr : recovered_addrs) {
            if (addr == 0)
                throw std::runtime_error("Concord decode: null recovered address");
            if (!unique_outputs.insert(addr).second)
                throw std::runtime_error("Concord decode: duplicate recovered address");
            if (unique_survivor_addrs.count(addr) != 0)
                throw std::runtime_error("Concord decode: recovered address aliases survivor");
        }

        std::lock_guard<std::mutex> lk(decode_mtx_);
        init_decode_tables_(k, survivor_positions, lost_positions);
        if (!decode_tbls_)
            throw std::runtime_error("Concord decode: failed to initialize decode tables");
    ...
    void send_to_peer(int peer_rig, int stripe_id, uintptr_t addr, size_t size,
        ...
        ConcordRdmaChannel* ch = get_channel_by_lane_(peer_rig, lane_id);
        if (!ch) {
            throw std::runtime_error(
                "Concord send_to_peer: no channel to rig " + std::to_string(peer_rig) +
    ...
    void recv_from_peer(int peer_rig, int stripe_id, uintptr_t addr, size_t size,
        ...
        if (!ch) {
            throw std::runtime_error(
                "Concord recv_from_peer: no channel from rig " + std::to_string(peer_rig) +
                " lane " + std::to_string(lane_id));
        }
        if (shared_lane_) {
    ...
    void send_layer_to_peer(int peer_rig, uintptr_t addr, size_t size, uint64_t batch_id, int lane_id = 0) {
        ...
        ConcordRdmaChannel* ch = get_channel_by_lane_(peer_rig, mapped_lane_id);
        if (!ch) {
            throw std::runtime_error(
                "Concord layer send: no channel to rig " + std::to_string(peer_rig) +
                " lane " + std::to_string(mapped_lane_id));
        }
        uint64_t net_t0 = concord_now_us();
        ...
        uint64_t net_t1 = concord_now_us();
    ...
    int get_max_send_sge() const {
        int result = CONCORD_DEFAULT_MAX_SEND_SGE;
        bool found = false;
    ...
    void send_layer_blocks_to_peer(
        ...
        if (block_size == 0 || block_indices.empty())
            throw std::runtime_error("Concord layer scatter send: empty block list or block size");
        if (block_indices.size() > SIZE_MAX / block_size)
            throw std::runtime_error("Concord layer scatter send: logical size overflow");
        int mapped_lane_id = map_save_send_lane_(peer_rig, lane_id);
        ConcordRdmaChannel* ch = get_channel_by_lane_(peer_rig, mapped_lane_id);
        if (!ch)
            throw std::runtime_error("Concord layer scatter send: no channel to rig " +
        ...
        for (size_t block_idx : block_indices) {
            if (block_idx > (SIZE_MAX - mirror_base) / block_size)
                throw std::runtime_error("Concord layer scatter send: block address overflow");
        ...
        while (stats_remaining > 0) {
            size_t message_remaining = std::min(stats_remaining, concord_rdma_chunk_size());
    ...
    void prepare_layer_recv_from_peer(
        ...
        ConcordRdmaChannel* ch = get_channel_by_lane_(peer_rig, mapped_lane_id);
        if (!ch) throw std::runtime_error("Concord prepared layer recv: no channel");
        if (!shared_lane_)
            throw std::runtime_error("Concord prepared layer recv requires shared lanes");
        record_save_net_start_(concord_now_us());
    ...
    size_t wait_prepared_layer_recv_from_peer(
        ...
        int mapped_lane_id = map_save_recv_lane_(peer_rig, lane_id);
        ConcordRdmaChannel* ch = get_channel_by_lane_(peer_rig, mapped_lane_id);
        if (!ch) throw std::runtime_error("Concord prepared layer recv wait: no channel");
        uint64_t net_t0 = concord_now_us();
        record_save_net_start_(net_t0);
    ...
    size_t recv_layer_from_peer(int peer_rig, uintptr_t addr, size_t capacity, uint64_t batch_id, int lane_id = 0) {
        ...
        if (!ch) {
            throw std::runtime_error(
                "Concord layer recv: no channel from rig " + std::to_string(peer_rig) +
                " lane " + std::to_string(mapped_lane_id));
        }
        uint64_t net_t0 = concord_now_us();
        record_save_net_start_(net_t0);
    ...
    void reset_recovery_generation() {
        ...
        if (pending_chunks != 0 || active_workers != 0)
            throw std::runtime_error(
                "Concord: recovery generation reset before drain: pending=" +
        ...
        reset_recovery_completion_();
        {
            ...
            if (recovery_generation_ >= kRecoveryGenerationMax)
                throw std::runtime_error("Concord: recovery generation exhausted");
    ...
    void init_recovery_plans(const std::vector<int>& failed_nodes_1based) {
        ...
        if (failed_nodes_1based.empty())
            throw std::runtime_error("Concord: init_recovery_plans requires failed nodes");
        if (failed_nodes_1based.size() > 2)
            throw std::runtime_error("Concord: at most 2 failed nodes per group");
        if (stripe_plans_.empty())
            throw std::runtime_error("Concord: compile_plans must run before init_recovery_plans");
    ...
    uint64_t begin_recovery_batch(bool low_priority = false) {
        if (encoding_batch_active_)
            throw std::runtime_error("Concord: encode batch active, cannot start recovery batch");
        ...
        uint64_t batch_id;
        {
            ...
            if (recovery_generation_ == 0 || next_recovery_batch_id_ >= generation_end)
                throw std::runtime_error("Concord: recovery batch ID exhausted for generation");
            batch_id = next_recovery_batch_id_++;
        }
        std::lock_guard<std::mutex> lk(recovery_batch_mtx_);
        RecoveryBatchState state;
        state.batch_start_us = concord_now_us();
    ...
    void reset_recovery_batch() {
        if (legacy_recovery_batch_active_)
            throw std::runtime_error("Concord: recovery batch already active");
    ...
    {
        if (stopped_)
            throw std::runtime_error("Concord recovery submit: native runtime is stopped");
        ensure_recovery_batch_exists_(batch_id);
        if (stripe_id < 0 || stripe_id >= (int)recovery_plans_.size())
            throw std::runtime_error("Concord: invalid recovery stripe_id");
        ...
        for (int role : target_roles) {
            if (role < (int)StripeRole::SOURCE || role > (int)StripeRole::PARITY_TARGET)
                throw std::runtime_error("Concord recovery submit: invalid target role");
            if (!requested_roles.insert(role).second)
                throw std::runtime_error("Concord recovery submit: duplicate target role");
        }
        ...
        if (selected_targets.size() > 2)
            throw std::runtime_error("Concord recovery submit: too many selected targets");
        ...
        if (is_helper() && helper_block_addr == 0)
            throw std::runtime_error("Concord recovery submit: missing helper block");
        if (is_helper()) {
        ...
        if (my_node == plan.decoder_node && decoder_self_block_addr == 0)
            throw std::runtime_error("Concord recovery submit: missing decoder self block");
        if (my_node == plan.decoder_node) {
            if (decoder_helper_recv_addrs.size() != plan.helper_nodes.size())
                throw std::runtime_error("Concord recovery submit: helper buffer count mismatch");
            if (decoder_recovered_addrs.size() < selected_targets.size())
                throw std::runtime_error("Concord recovery submit: recovered buffer count mismatch");
        ...
        for (const RecoveryFailedTarget* ft : selected_targets) {
            ...
            if (failed_recv_buf_addr == 0)
                throw std::runtime_error("Concord recovery submit: missing failed receive buffer");
    ...
    {
        if (!legacy_recovery_batch_active_)
            throw std::runtime_error("Concord: submit_recovery_stripe without reset_recovery_batch");
    ...
    void end_recovery_batch(uint64_t batch_id) {
        {
            ...
            if (it == recovery_batches_.end())
                throw std::runtime_error("Concord: unknown recovery batch id");
    ...
    void submit_recovery_sentinel() {
        if (!legacy_recovery_batch_active_)
            throw std::runtime_error("Concord: submit_recovery_sentinel without reset_recovery_batch");
        end_recovery_batch(legacy_recovery_batch_id_);
    }

    void wait_recovery_batch() {
        if (!legacy_recovery_batch_active_)
            throw std::runtime_error("Concord: wait_recovery_batch without reset_recovery_batch");
...
private:
    void register_buffer_(uintptr_t addr, size_t size, bool recovery_buffer) {
        ...
        if (!mr) {
            if (require_registered_mr_) {
                throw std::runtime_error(
                    "Concord RDMA: ibv_reg_mr failed for save pipeline (addr=0x" +
                    std::to_string(addr) + " size=" + std::to_string(size) +
                    ", GPU pointers need nvidia-peermem for GDR)");
            }
            std::cerr << "[Concord RDMA] WARNING: ibv_reg_mr failed for addr=0x"
    ...
    void load_poa_(std::istream& in) {
        ...
        while (std::getline(in, line)) {
            ...
            if (n_ < 0) {
                n_ = (int)rv.size();
                if (n_ < 3)
                    throw std::runtime_error("Concord: POA must have >=3 columns");
            } else if ((int)rv.size() != n_) {
                throw std::runtime_error("Concord: inconsistent row width");
            }
            for (int x : rv)
                if (x < 1 || x > n_)
                    throw std::runtime_error("Concord: entry out of range [1,n]");
            table_.push_back(std::move(rv));
        }
        if (n_ < 0 || table_.empty())
            throw std::runtime_error("Concord: empty POA file");
        int expected = n_ * (n_ - 1);
        if ((int)table_.size() != expected)
            throw std::runtime_error("Concord: expected " + std::to_string(expected) +
                                     " rows, got " + std::to_string(table_.size()));
    }

    // ---- POA auto-generation ----
    int parse_n_from_path_(const std::string& path) {
        // Parse n from common filename patterns: poa_n{}.txt, n{}.poa, concord_poa_n{}.txt
        ...
        throw std::runtime_error("Concord: cannot determine n from filename: " + path +
    ...
    void generate_poa_(int n) {
        if (n < 3) throw std::runtime_error("Concord: n must be >= 3");
        table_.clear();
        n_ = n;
        ...
        if (is_prime_(n)) {
            ...
            if (poly == 0)
                throw std::runtime_error("Concord: no primitive polynomial for GF(" +
                                         std::to_string(n) + ")");
            ...
            throw std::runtime_error(
                "Concord: cannot auto-generate POA for n=" + std::to_string(n) +
        ...
        int expected = n_ * (n_ - 1);
        if ((int)table_.size() != expected)
            throw std::runtime_error("Concord generate: expected " +
    ...
    void init_encode_tables_() {
        ...
        // ISA-L owns 32 bytes of encoding table per source/row pair.
        ...
        if (!g_tbls_) {
            ...
            throw std::runtime_error("Concord: failed to alloc encode tables");
        }
    ...
    void init_decode_tables_(int k,
        ...
        if (lost_positions.empty() || lost_positions.size() > static_cast<size_t>(m_parity))
            throw std::runtime_error("Concord decode table: invalid lost position count");

        std::set<int> unique_survivors;
        for (int pos : survivor_positions) {
            if (pos < 0 || pos >= full_rows)
                throw std::runtime_error("Concord decode table: survivor position out of range");
            if (!unique_survivors.insert(pos).second)
                throw std::runtime_error("Concord decode table: duplicate survivor position");
        }
        std::set<int> unique_losses;
        for (int pos : lost_positions) {
            if (pos < 0 || pos >= full_rows)
                throw std::runtime_error("Concord decode table: lost position out of range");
            if (!unique_losses.insert(pos).second)
                throw std::runtime_error("Concord decode table: duplicate lost position");
            if (unique_survivors.count(pos) != 0)
                throw std::runtime_error("Concord decode table: lost position listed as survivor");
        }
        ...
        if (ret != 0) {
            throw std::runtime_error(
                "Concord decode table: gf_invert_matrix failed, ret=" +
                std::to_string(ret));
        }
        ...
        if (tmp == nullptr)
            throw std::runtime_error("Concord decode table: allocation failed");
    ...
    void rs_pool_init() {
        ...
        for (int i = 0; i < kRsPoolSize; ++i) {
            ...
            int rc = pthread_create(&rs_pool_threads_[i], nullptr,
                                    &ConcordNative::rs_pool_entry, &rs_pool_ctx_[i]);
            if (rc != 0) {
                ...
                throw std::runtime_error("Concord: pthread_create for RS pool failed: " +
                                         std::string(std::strerror(rc)));
            }
        }
        rs_pool_inited_.store(true, std::memory_order_release);
        if (debug_)
            std::cout << "Concord: RS encode pool (" << kRsPoolSize << " workers) initialized" << std::endl;
    }
    ...
    void rs_pool_shutdown() {
        ...
        if (debug_)
            std::cout << "Concord: RS encode pool shut down" << std::endl;
    }
    ...
    void rs_pool_worker(int wid) {
        ...
        while (true) {
            ...
            if (jobs != nullptr) {
                const uint64_t compute_t0 = concord_now_us();
                rs_pool_execute_batch_slice(*jobs, wid);
                rs_pool_worker_elapsed_us_[wid].store(
                    concord_now_us() - compute_t0, std::memory_order_release);
            } else {
    ...
    void rs_pool_build_batch_plan(const std::vector<RsEncodeJob>& jobs) {
        ...
        for (const auto& job : jobs) {
            if (job.len < 0 || job.k <= 0 || job.k > kRsMaxDataBlocks ||
                ...
                job.g_tbls == nullptr || job.data_ptrs == nullptr ||
                job.parity_ptrs == nullptr) {
                throw std::runtime_error("Concord: invalid RS batch job");
            }
            for (int i = 0; i < job.k; ++i) {
                if (job.data_ptrs[i] == nullptr)
                    throw std::runtime_error("Concord: null RS batch source pointer");
            }
            for (int i = 0; i < job.m; ++i) {
                if (job.parity_ptrs[i] == nullptr)
                    throw std::runtime_error("Concord: null RS batch parity pointer");
            }
            const uint64_t len = static_cast<uint64_t>(job.len);
            if (total_bytes > std::numeric_limits<uint64_t>::max() - len)
                throw std::runtime_error("Concord: RS batch byte count overflow");
            total_bytes += len;
        }
            ...
            while (scan_index < jobs.size() && scan_begin < worker_end) {
                ...
                if (begin < end) {
                    ...
                    if (scan_index > std::numeric_limits<uint32_t>::max() ||
                        ...
                        throw std::runtime_error("Concord: RS batch span exceeds supported range");
                    }
    ...
    void rs_pool_run_parallel_encode_batch(const std::vector<RsEncodeJob>& jobs) {
        ...
        const uint64_t plan_t0 = concord_now_us();
        rs_pool_build_batch_plan(jobs);
        save_encode_pool_plan_total_us_.fetch_add(
            concord_now_us() - plan_t0, std::memory_order_relaxed);
        for (auto& elapsed : rs_pool_worker_elapsed_us_)
            elapsed.store(0, std::memory_order_relaxed);
        const uint64_t barrier_t0 = concord_now_us();
        {
            std::lock_guard<std::mutex> publish(rs_pool_mutex_);
            if (stopped_) return;
        ...
        const uint64_t barrier_us = concord_now_us() - barrier_t0;
    ...
    // Source, encoder-receive, encoder-send, and parity queues each own n workers.
    struct StripeInfo {
    ...
    struct EncRecvTask  { int sid; uintptr_t recv, primary_parity, deferred_parity; size_t bs; std::vector<uint8_t> mask; uint64_t encode_batch = 0; };
    ...
    struct EncSendTask  { int sid; uintptr_t recv, primary_parity, deferred_parity; size_t bs; int n_src; std::vector<int> src_peer_rigs; int par_peer_rig; bool parity_send_only = false; uint64_t async_order = 0; uint64_t async_batch = 0; };
    struct ParityTask   { int sid; uintptr_t parity_in; size_t bs; bool aggregate_parity_only = false; uint64_t async_order = 0; uint64_t async_batch = 0; };
    ...
    std::vector<std::thread> aggregate_parity_threads_;
    std::mutex aggregate_parity_mtx_;
    std::condition_variable aggregate_parity_cv_;
    std::exception_ptr aggregate_parity_error_;
    std::atomic<int> aggregate_parity_total_{0};
    std::atomic<int> aggregate_parity_done_{0};
    uint64_t aggregate_parity_generation_ = 0;
    std::mutex encode_done_mtx_;
    std::condition_variable encode_done_cv_;
    std::atomic<int> async_parity_pause_count_{0};
    std::mutex async_parity_send_order_mtx_;
    std::mutex async_parity_order_mtx_;
    std::condition_variable async_parity_order_cv_;
    std::vector<uint64_t> async_parity_send_enq_order_;
    std::vector<uint64_t> async_parity_send_run_order_;
    std::vector<uint64_t> async_parity_recv_enq_order_;
    std::vector<uint64_t> async_parity_recv_run_order_;
    std::vector<uint64_t> async_parity_send_batch_run_order_;
    std::vector<uint64_t> async_parity_recv_batch_run_order_;
    ...
    std::atomic<int> async_parity_inflight_{0};
    std::mutex async_parity_inflight_mtx_;
    std::condition_variable async_parity_inflight_cv_;
    bool debug_ = false;
    ...
    void _wait_if_paused() {
        while (async_parity_pause_count_.load(std::memory_order_acquire) > 0) {
            if (debug_) {
                static int log_counter = 0;
                if (++log_counter % 1000 == 1)
                    std::cerr << "[CONCORD-DEBUG] rank " << rank_in_group_
                              << " async P2 paused (refcount="
                              << async_parity_pause_count_.load() << ")" << std::endl;
            }
    ...
    void _async_rdma_begin() {
        while (true) {
            async_parity_inflight_.fetch_add(1, std::memory_order_acq_rel);
            if (async_parity_pause_count_.load(std::memory_order_acquire) == 0) {
                return;
            }
            _async_rdma_end();
            _wait_if_paused();
    ...
    void _async_rdma_end() {
        int prev = async_parity_inflight_.fetch_sub(1, std::memory_order_acq_rel);
        if (prev <= 1) {
            std::lock_guard<std::mutex> lk(async_parity_inflight_mtx_);
            async_parity_inflight_cv_.notify_all();
        }
    }

    void _wait_async_rdma_idle() {
        if (async_parity_inflight_.load(std::memory_order_acquire) == 0) return;
        std::unique_lock<std::mutex> lk(async_parity_inflight_mtx_);
        async_parity_inflight_cv_.wait(lk, [&] {
            return async_parity_inflight_.load(std::memory_order_acquire) == 0;
        });
    }

    void wait_async_parity_send_turn_(int sid, uint64_t order, uint64_t batch = 0) {
        std::unique_lock<std::mutex> lk(async_parity_order_mtx_);
        while (!all_stop_.load(std::memory_order_acquire) &&
               ((batch == 0 && sid >= 0 && sid < (int)async_parity_send_run_order_.size() &&
                 async_parity_send_run_order_[(size_t)sid] != order) ||
                (batch != 0 && sid >= 0 && sid < (int)async_parity_send_batch_run_order_.size() &&
                 async_parity_send_batch_run_order_[(size_t)sid] != batch))) {
            if (debug_) {
                std::cerr << "[CONCORD-DEBUG] rank " << rank_in_group_
                          << " async_p2_send wait_turn sid=" << sid
                          << " order=" << order
                          << " batch=" << batch
                          << " expect=" << (sid >= 0 && sid < (int)async_parity_send_run_order_.size() ? async_parity_send_run_order_[(size_t)sid] : 0)
                          << std::endl;
            }
            async_parity_order_cv_.wait_for(lk, std::chrono::seconds(1));
        }
        if (debug_) {
            uint64_t expect = (sid >= 0 && sid < (int)async_parity_send_run_order_.size())
                                ? async_parity_send_run_order_[(size_t)sid] : 0;
            std::cerr << "[CONCORD-DEBUG] rank " << rank_in_group_
    ...
    void advance_async_parity_send_turn_(int sid, uint64_t batch = 0) {
        {
            std::lock_guard<std::mutex> lk(async_parity_order_mtx_);
            if (batch == 0) {
                if (sid >= 0 && sid < (int)async_parity_send_run_order_.size())
                    ++async_parity_send_run_order_[(size_t)sid];
            } else {
                if (sid >= 0 && sid < (int)async_parity_send_batch_run_order_.size())
                    ++async_parity_send_batch_run_order_[(size_t)sid];
            }
        }
        async_parity_order_cv_.notify_all();
    }

    void wait_async_parity_recv_turn_(int sid, uint64_t order, uint64_t batch = 0) {
        std::unique_lock<std::mutex> lk(async_parity_order_mtx_);
        while (!all_stop_.load(std::memory_order_acquire) &&
               ((batch == 0 && sid >= 0 && sid < (int)async_parity_recv_run_order_.size() &&
                 async_parity_recv_run_order_[(size_t)sid] != order) ||
                (batch != 0 && sid >= 0 && sid < (int)async_parity_recv_batch_run_order_.size() &&
                 async_parity_recv_batch_run_order_[(size_t)sid] != batch))) {
            if (debug_) {
                std::cerr << "[CONCORD-DEBUG] rank " << rank_in_group_
                          << " async_p2_recv wait_turn sid=" << sid
                          << " order=" << order
                          << " batch=" << batch
                          << " expect=" << (sid >= 0 && sid < (int)async_parity_recv_run_order_.size() ? async_parity_recv_run_order_[(size_t)sid] : 0)
                          << std::endl;
            }
            async_parity_order_cv_.wait_for(lk, std::chrono::seconds(1));
        }
        if (debug_) {
            uint64_t expect = (sid >= 0 && sid < (int)async_parity_recv_run_order_.size())
                                ? async_parity_recv_run_order_[(size_t)sid] : 0;
            std::cerr << "[CONCORD-DEBUG] rank " << rank_in_group_
    ...
    void advance_async_parity_recv_turn_(int sid, uint64_t batch = 0) {
        {
            std::lock_guard<std::mutex> lk(async_parity_order_mtx_);
            if (batch == 0) {
                if (sid >= 0 && sid < (int)async_parity_recv_run_order_.size())
                    ++async_parity_recv_run_order_[(size_t)sid];
            } else {
                if (sid >= 0 && sid < (int)async_parity_recv_batch_run_order_.size())
                    ++async_parity_recv_batch_run_order_[(size_t)sid];
            }
        }
        async_parity_order_cv_.notify_all();
    }
    ...
    void source_worker_() {
        while (!all_stop_) {
            ...
            if (!t.skip) {
                ...
                if (!ch) { std::cerr << "Concord source " << t.sid << ": no channel\n"; continue; }
                uint64_t net_t0 = concord_now_us();
                record_save_net_start_(net_t0);
                if (shared_lane_) {
    ...
    void enc_recv_worker_() {
        while (!all_stop_) {
            ...
            // Transfer ownership to the encoder-send queue.
            EncSendTask es{t.sid, t.recv, t.primary_parity, t.deferred_parity, t.bs, n_src, si.src_peer_rigs, si.par_peer_rig};
    ...
    void enc_send_worker_() {
        while (!all_stop_) {
            ...
            if (t.parity_send_only) {
                wait_async_parity_send_turn_(t.sid, t.async_order, t.async_batch);
                _wait_if_paused();
                    ...
                    if (ch) {
                        ...
                        if (shared_lane_) {
                            ch->send_tagged(make_channel_tag_(1, t.sid, t.async_batch),
                                            (const uint8_t*)t.primary_parity, t.bs, wait_cb, done_cb);
                        } else {
                            ch->send_data((const uint8_t*)t.primary_parity, t.bs, wait_cb, done_cb);
                        }
                        record_save_net_end_(concord_now_us());
                    }
                }
                advance_async_parity_send_turn_(t.sid, t.async_batch);
                check_async_done_();
                if (debug_) {
                    std::cerr << "[CONCORD-DEBUG] rank " << rank_in_group_
    ...
    void parity_worker_() {
        while (!all_stop_) {
            ...
            if (debug_ && t.aggregate_parity_only) {
                std::cerr << "[CONCORD-DEBUG] rank " << rank_in_group_
            ...
            if (ch) {
                uint64_t net_t0 = concord_now_us();
                record_save_net_start_(net_t0);
                if (shared_lane_ && t.aggregate_parity_only) {
            ...
            if (t.aggregate_parity_only) advance_async_parity_recv_turn_(t.sid, t.async_batch);
            check_async_done_();
            if (debug_ && t.aggregate_parity_only) {
                std::cerr << "[CONCORD-DEBUG] rank " << rank_in_group_
    ...
    void mirror_worker_func() {
        ...
        while (true) {
            ...
            if (err != cudaSuccess) {
                ...
                std::cerr << "Concord mirror_worker: async D2H failed: "
...
public:
    ...
    void encode_layer_stripes_batch(
        const std::vector<int>& stripe_ids, const std::vector<uintptr_t>& data_addrs,
        const std::vector<uintptr_t>& primary_parity_addrs, const std::vector<uintptr_t>& deferred_parity_addrs,
        const std::vector<size_t>& block_sizes) {
        ...
        for (size_t i = 0; i < stripe_ids.size(); ++i) {
            parity_addrs.push_back(primary_parity_addrs.at(i));
            parity_addrs.push_back(deferred_parity_addrs.at(i));
        }
    ...
    void encode_layer_stripes(
        ...
        const std::vector<uintptr_t>& primary_parity_addrs, const std::vector<uintptr_t>& deferred_parity_addrs,
        size_t block_size) {
        encode_layer_stripes_batch(stripe_ids, data_addrs, primary_parity_addrs, deferred_parity_addrs,
                                   std::vector<size_t>(stripe_ids.size(), block_size));
    }
    ...
    void finalize_mirror_stream_() {
        if (d2h_stream_ != nullptr) {
            ...
            if (err != cudaSuccess) {
                ...
                d2h_stream_ = nullptr;
                throw std::runtime_error(
                    std::string("Concord mirror: d2h_stream sync failed: ") +
        ...
        if (mirror_thread_.joinable()) {
            ...
            while (!mirror_done_.load()) {
                if (std::chrono::steady_clock::now() > deadline) {
                    std::cerr << "Concord mirror: worker drain timed out on rank "
...
public:
    void start_mirror_worker() {
        ...
        if (err != cudaSuccess) {
            throw std::runtime_error(
                std::string("Concord: cudaStreamCreate(d2h_stream) failed: ") +
                cudaGetErrorString(err));
        }
        mirror_thread_ = std::thread(&ConcordNative::mirror_worker_func, this);
    }
...
private:
    void init_stripe_workers() {
        ...
        async_parity_send_batch_run_order_.assign((size_t)ns, 1);
        async_parity_recv_batch_run_order_.assign((size_t)ns, 1);
        encode_source_batch_run_order_.assign((size_t)ns, 1);
        encode_recv_batch_run_order_.assign((size_t)ns, 1);
        async_parity_send_enq_order_.assign((size_t)ns, 0);
        async_parity_send_run_order_.assign((size_t)ns, 0);
        async_parity_recv_enq_order_.assign((size_t)ns, 0);
        async_parity_recv_run_order_.assign((size_t)ns, 0);
        for (int i = 0; i < ns; ++i) {
        ...
        int nw = n_;
        // Encoder receive subworkers cover up to n concurrent stripes and n-2 sources each.
        int enc_recv_part_nw = nw * std::max(1, nw - 2);
        const char* part_env = std::getenv("CONCORD_ENC_RECV_PART_WORKERS");
        if (part_env && part_env[0] != '\0') {
            int env_nw = std::atoi(part_env);
            if (env_nw > 0) enc_recv_part_nw = env_nw;
        }
        for (int w = 0; w < nw; ++w) {
            source_workers_.emplace_back(&ConcordNative::source_worker_, this);
            enc_recv_workers_.emplace_back(&ConcordNative::enc_recv_worker_, this);
            enc_send_workers_.emplace_back(&ConcordNative::enc_send_worker_, this);
            parity_workers_.emplace_back(&ConcordNative::parity_worker_, this);
        }
        for (int w = 0; w < enc_recv_part_nw; ++w) {
            enc_recv_part_workers_.emplace_back(&ConcordNative::enc_recv_part_worker_, this);
        }
        // Deferred parity sends share the n encoder-send workers and their FIFO queue.
        if (debug_)
            std::cout << "Concord: " << nw << " workers/role, "
...
public:
    ...
    void submit_enc_recv_with_batch(int sid, uintptr_t recv, uintptr_t primary_parity_addr, uintptr_t deferred_parity_addr, size_t bs,
                                    const std::vector<uint8_t>& mask, uint64_t batch) {
        if (m_ != 2)
            throw std::runtime_error("Concord legacy stripe worker API requires m=2");
        task_total_.fetch_add(1, std::memory_order_acq_rel);
        task_encode_total_.fetch_add(1, std::memory_order_acq_rel);
        { std::lock_guard<std::mutex> lk(enc_recv_mtx_); enc_recv_q_.push({sid, recv, primary_parity_addr, deferred_parity_addr, bs, mask, batch}); }
        enc_recv_cv_.notify_one();
    }

    void submit_enc_recv(int sid, uintptr_t recv, uintptr_t primary_parity_addr, uintptr_t deferred_parity_addr, size_t bs,
                         const std::vector<uint8_t>& mask) {
        submit_enc_recv_with_batch(sid, recv, primary_parity_addr, deferred_parity_addr, bs, mask, 0);
    }
    ...
    void submit_deferred_parity_send(int sid, uintptr_t parity_addr, size_t bs) {
        auto& si = stripe_info_[(size_t)sid];
        EncSendTask t;
        t.sid = sid;
        t.primary_parity = parity_addr;
        t.bs = bs;
    ...
    // Legacy two-parity API aliases; both submit an already-selected parity address.
    void submit_p1_send(int sid, uintptr_t p1_addr, size_t bs) {
        submit_deferred_parity_send(sid, p1_addr, bs);
    }

    void submit_p2_send(int sid, uintptr_t p2_addr, size_t bs) {
        submit_deferred_parity_send(sid, p2_addr, bs);
    }

    void submit_async_deferred_parity_batch(
        const std::vector<std::tuple<int, uintptr_t, size_t>>& parity_tasks,
        const std::vector<std::tuple<int, uintptr_t, size_t>>& deferred_send_tasks) {
        int total = static_cast<int>(parity_tasks.size() + deferred_send_tasks.size());
        if (total == 0) return;
        ...
        if (!deferred_send_tasks.empty()) {
            std::lock_guard<std::mutex> lk(enc_send_mtx_);
            for (const auto& task : deferred_send_tasks) {
                int sid;
                uintptr_t p2_addr;
                size_t bs;
        ...
        if (!deferred_send_tasks.empty()) enc_send_cv_.notify_all();
    }

    std::vector<int> submit_async_deferred_parity_layer_with_batch(
        const std::vector<uintptr_t>& deferred_parity_addrs,
        size_t bs,
        uint64_t batch) {
        int parity_count = 0;
        int send_count = 0;
        int ns = std::min((int)deferred_parity_addrs.size(), (int)stripe_info_.size());
        for (int sid = 0; sid < ns; ++sid) {
            if (deferred_parity_addrs[(size_t)sid] == 0) continue;
        ...
        if (parity_count > 0) {
            ...
            for (int sid = 0; sid < ns; ++sid) {
                uintptr_t addr = deferred_parity_addrs[(size_t)sid];
                if (addr == 0) continue;
                if (get_role_for_stripe(sid) == (int)StripeRole::PARITY_TARGET) {
                    ...
                    t.parity_in = addr;
                    t.bs = bs;
                    t.aggregate_parity_only = true;
                    t.async_batch = batch;
                    {
                        std::lock_guard<std::mutex> order_lk(async_parity_order_mtx_);
                        if (batch == 0) {
                            if (sid >= 0 && sid < (int)async_parity_recv_enq_order_.size())
                                t.async_order = async_parity_recv_enq_order_[(size_t)sid]++;
                        } else {
                            t.async_order = batch;
                        }
                    }
            ...
            for (int sid = 0; sid < ns; ++sid) {
                ...
                if (get_role_for_stripe(sid) == (int)StripeRole::ENCODER) {
                    ...
                    t.async_batch = batch;
                    {
                        std::lock_guard<std::mutex> order_lk(async_parity_order_mtx_);
                        if (batch == 0) {
                            if (sid >= 0 && sid < (int)async_parity_send_enq_order_.size())
                                t.async_order = async_parity_send_enq_order_[(size_t)sid]++;
                        } else {
                            t.async_order = batch;
                        }
                    }
    ...
    std::vector<int> submit_async_deferred_parity_layer(
        const std::vector<uintptr_t>& deferred_parity_addrs,
        size_t bs) {
        return submit_async_deferred_parity_layer_with_batch(
            deferred_parity_addrs, bs, 0);
    }
    ...
    std::vector<int> get_deferred_parity_route(int sid) const {
        if (m_ < 2) return {(int)StripeRole::INACTIVE, -1, -1};
        return get_parity_route(sid, 1);
    }

    using AggregateParityTask = std::tuple<
        int, int, std::vector<std::pair<uintptr_t, size_t>>>;

    std::vector<int> submit_aggregate_parity(
        const std::vector<AggregateParityTask>& send_tasks,
        const std::vector<AggregateParityTask>& recv_tasks,
        uint64_t generation) {
        if (!aggregate_parity_threads_.empty() ||
            aggregate_parity_done_.load(std::memory_order_acquire) <
                aggregate_parity_total_.load(std::memory_order_acquire)) {
            throw std::runtime_error("Concord aggregate P2: previous generation is still active");
        }
        if (generation == 0 || generation <= aggregate_parity_generation_)
            throw std::runtime_error("Concord aggregate P2: generation must increase");
        aggregate_parity_generation_ = generation;
        aggregate_parity_error_ = nullptr;
        aggregate_parity_done_.store(0, std::memory_order_release);
        const int total = static_cast<int>(send_tasks.size() + recv_tasks.size());
        aggregate_parity_total_.store(total, std::memory_order_release);

        auto launch = [this, generation](
                bool is_send, int peer, int lane,
                std::vector<std::pair<uintptr_t, size_t>> segments) {
            aggregate_parity_threads_.emplace_back([
                    this, generation, is_send, peer, lane,
                    segments = std::move(segments)]() {
                try {
                    ConcordRdmaChannel* ch = get_channel_by_lane_(peer, lane);
                    if (!ch)
                        throw std::runtime_error(
                            "Concord aggregate P2: missing channel peer=" +
                            std::to_string(peer) + " lane=" + std::to_string(lane));
                    size_t size = 0;
                    for (const auto& segment : segments) {
                        if (segment.second > std::numeric_limits<size_t>::max() - size)
                            throw std::runtime_error("Concord aggregate P2: segment size overflow");
                        size += segment.second;
                    }
                    ...
                    if (is_send) {
                        ...
                        if (got != size)
                            throw std::runtime_error("Concord aggregate P2: short tagged receive");
                    } else {
                        const size_t got = ch->recv_segments(segments, size);
                        if (got != size)
                            throw std::runtime_error("Concord aggregate P2: short receive");
                    }
                    record_save_net_end_(concord_now_us());
                } catch (...) {
                    std::lock_guard<std::mutex> lk(aggregate_parity_mtx_);
                    if (!aggregate_parity_error_) aggregate_parity_error_ = std::current_exception();
                }
                aggregate_parity_done_.fetch_add(1, std::memory_order_acq_rel);
                aggregate_parity_cv_.notify_all();
            });
        };
        try {
            ...
            abort_all_channels_();
            for (auto& thread : aggregate_parity_threads_)
                if (thread.joinable()) thread.join();
            aggregate_parity_threads_.clear();
            aggregate_parity_total_.store(0, std::memory_order_release);
            aggregate_parity_done_.store(0, std::memory_order_release);
            throw;
        }
    ...
    void wait_aggregate_parity(int timeout_seconds = 300) {
        const int total = aggregate_parity_total_.load(std::memory_order_acquire);
        if (total > 0) {
            std::unique_lock<std::mutex> lk(aggregate_parity_mtx_);
            if (!aggregate_parity_cv_.wait_for(
                    lk, std::chrono::seconds(std::max(1, timeout_seconds)), [this, total] {
                        return aggregate_parity_done_.load(std::memory_order_acquire) >= total;
                    })) {
                const int done = aggregate_parity_done_.load(std::memory_order_acquire);
                lk.unlock();
                abort_all_channels_();
                for (auto& thread : aggregate_parity_threads_)
                    if (thread.joinable()) thread.join();
                aggregate_parity_threads_.clear();
                throw std::runtime_error(
                    "Concord aggregate P2 timed out: done=" +
        ...
        {
            std::lock_guard<std::mutex> lk(aggregate_parity_mtx_);
            error = aggregate_parity_error_;
            aggregate_parity_error_ = nullptr;
        }
        if (error) std::rethrow_exception(error);
        if (aggregate_parity_done_.load(std::memory_order_acquire) != total)
            throw std::runtime_error("Concord aggregate P2 completion count mismatch");
    }
    ...
    void reset_async_parity() {
        ...
        {
            std::lock_guard<std::mutex> lk(async_parity_order_mtx_);
            std::fill(async_parity_send_enq_order_.begin(), async_parity_send_enq_order_.end(), 0);
            std::fill(async_parity_send_run_order_.begin(), async_parity_send_run_order_.end(), 0);
            std::fill(async_parity_recv_enq_order_.begin(), async_parity_recv_enq_order_.end(), 0);
            std::fill(async_parity_recv_run_order_.begin(), async_parity_recv_run_order_.end(), 0);
            std::fill(async_parity_send_batch_run_order_.begin(), async_parity_send_batch_run_order_.end(), 1);
            std::fill(async_parity_recv_batch_run_order_.begin(), async_parity_recv_batch_run_order_.end(), 1);
        }
        async_parity_order_cv_.notify_all();
    }
    ...
    void wait_layer() {
        ...
        if (task_done_.load() < total)
            std::cerr << "[CONCORD-DIAG] rank " << rank_in_group_ << " stuck: done="
    ...
    void inc_pause_async_p2p() {
        // The pause count gates new asynchronous parity RDMA; in-flight work retains ownership until completion.
        int prev = async_parity_pause_count_.fetch_add(1, std::memory_order_acq_rel);
        if (debug_)
            std::cerr << "[CONCORD-DEBUG] rank " << rank_in_group_
    ...
    void dec_pause_async_p2p() {
        int current = async_parity_pause_count_.load(std::memory_order_acquire);
        while (current > 0) {
            if (async_parity_pause_count_.compare_exchange_weak(
                    current, current - 1,
    ...
    void compile_recovery_plans_dual_(const std::vector<int>& failed_nodes) {
        if (k_ != n_ - 2 || m_ != 2)
            throw std::runtime_error("Concord dual-failure recovery (HW2 mode) requires the default k=n-2, m=2 layout");
        ...
        for (const auto& sp : stripe_plans_) {
            ...
            for (int fn : failed_nodes) {
                ...
                if (it == row.end()) {
                    throw std::runtime_error(
                        "Concord dual recovery: failed node " + std::to_string(fn) +
            ...
            if ((int)plan.survivor_positions.size() != n_ - 2) {
                throw std::runtime_error(
                    "Concord dual recovery: stripe " + std::to_string(sid) +
    ...
    void send_recovery_to_peer_(int peer_rig, int stripe_id, uintptr_t addr,
        ...
        struct AsyncRdmaScope {
            ConcordNative* owner;
            explicit AsyncRdmaScope(ConcordNative* native) : owner(native) {
                const uint64_t wait_start = concord_now_us();
                owner->_wait_if_paused();
                owner->_async_rdma_begin();
                const uint64_t waited = concord_now_us() - wait_start;
                ...
                if (owner->debug_ && waited > 0) {
                    std::cerr << "[CONCORD-DEBUG] rank " << owner->rank_in_group_
    ...
    void execute_recovery_decoder_(const RecoveryDecoderTask& task) {
        uint64_t t_recv = concord_now_us();
            ...
            recv_threads.emplace_back([&, hi]() {
                try {
                    if (hi >= task.helper_recv_bufs.size())
                        throw std::runtime_error("Concord recovery: missing helper recv buf");
                    recv_from_peer(
        ...
        uint64_t t_recv_done = concord_now_us();
        ...
        if (!task.dual_failure) {
            if (task.recovered_bufs.empty())
                throw std::runtime_error("Concord recovery: missing recovered buffer");
            uint64_t t_decode = concord_now_us();
            ...
            uint64_t t_decode_done = concord_now_us();
            ...
            if (task.failed_rigs.empty())
                throw std::runtime_error("Concord recovery: missing failed rig");
            ...
            if (task.failed_positions.empty() || task.failed_positions.size() > 2 ||
                ...
                throw std::runtime_error("Concord recovery: invalid filtered dual outputs");
            }
            uint64_t t_decode = concord_now_us();
            record_recovery_decode_start_(t_decode);
            submit_stripe_decode_batch(
    ...
    void execute_recovery_decoder_send_(const RecoveryDecoderSendTask& task) {
        uint64_t t_send = concord_now_us();
        ...
        uint64_t t_send_done = concord_now_us();
    ...
    void execute_recovery_failed_(const RecoveryFailedTask& task) {
        ...
        if (task.store_to_layer && task.layer_buf != 0 && task.ncopy > 0) {
            uint64_t t_copy = concord_now_us();
            std::memcpy(
                reinterpret_cast<void*>(task.layer_buf + task.layer_offset),
                reinterpret_cast<const void*>(task.recv_buf),
                task.ncopy);
            recovery_failed_copy_us_.fetch_add(concord_now_us() - t_copy, std::memory_order_relaxed);
        }
        recovery_batch_record_event_(
            task.batch_id, RecoveryBatchEvent::FAILED_DELIVERED, concord_now_us());
    ...
    void helper_worker_loop_() {
        while (!recovery_workers_stop_) {
            ...
            try {
                ...
                std::cerr << "Concord helper_worker: " << e.what() << std::endl;
                recovery_batch_mark_error_(task.batch_id, "Concord helper_worker: " + std::string(e.what()));
                continue;
            } catch (...) {
                helper_active_.fetch_sub(1, std::memory_order_acq_rel);
                pending_recovery_chunks_.fetch_sub(1, std::memory_order_acq_rel);
                if (recovery_workers_stop_) return;
                recovery_batch_mark_error_(task.batch_id, "Concord helper_worker: unknown exception");
                continue;
            }
            helper_active_.fetch_sub(1, std::memory_order_acq_rel);
            pending_recovery_chunks_.fetch_sub(1, std::memory_order_acq_rel);
    ...
    void decoder_worker_loop_() {
        while (!recovery_workers_stop_) {
            ...
            try {
                ...
                if (recovery_workers_stop_) return;
                std::cerr << "Concord decoder_worker: " << e.what() << std::endl;
                recovery_batch_mark_error_(task.batch_id, "Concord decoder_worker: " + std::string(e.what()), task.completion_count);
                continue;
            } catch (...) {
                decoder_active_.fetch_sub(1, std::memory_order_acq_rel);
                pending_recovery_chunks_.fetch_sub(task.completion_count, std::memory_order_acq_rel);
                if (recovery_workers_stop_) return;
                recovery_batch_mark_error_(task.batch_id, "Concord decoder_worker: unknown exception", task.completion_count);
                continue;
            }
            decoder_active_.fetch_sub(1, std::memory_order_acq_rel);
        }
    ...
    void decoder_send_worker_loop_() {
        while (!recovery_workers_stop_) {
            ...
            try {
                ...
                pending_recovery_chunks_.fetch_sub(1, std::memory_order_acq_rel);
                if (recovery_workers_stop_) return;
                std::cerr << "Concord decoder_send_worker: " << e.what() << std::endl;
                recovery_batch_mark_error_(task.batch_id, "Concord decoder_send_worker: " + std::string(e.what()));
                continue;
            } catch (...) {
                decoder_send_active_.fetch_sub(1, std::memory_order_acq_rel);
                pending_recovery_chunks_.fetch_sub(1, std::memory_order_acq_rel);
                if (recovery_workers_stop_) return;
                recovery_batch_mark_error_(task.batch_id, "Concord decoder_send_worker: unknown exception");
                continue;
            }
            decoder_send_active_.fetch_sub(1, std::memory_order_acq_rel);
            pending_recovery_chunks_.fetch_sub(1, std::memory_order_acq_rel);
    ...
    void failed_worker_loop_() {
        while (!recovery_workers_stop_) {
            ...
            try {
                ...
                pending_recovery_chunks_.fetch_sub(1, std::memory_order_acq_rel);
                if (recovery_workers_stop_) return;
                std::cerr << "Concord failed_worker: " << e.what() << std::endl;
                recovery_batch_mark_error_(task.batch_id, "Concord failed_worker: " + std::string(e.what()));
                continue;
            } catch (...) {
                failed_active_.fetch_sub(1, std::memory_order_acq_rel);
                pending_recovery_chunks_.fetch_sub(1, std::memory_order_acq_rel);
                if (recovery_workers_stop_) return;
                recovery_batch_mark_error_(task.batch_id, "Concord failed_worker: unknown exception");
                continue;
            }
            failed_active_.fetch_sub(1, std::memory_order_acq_rel);
            pending_recovery_chunks_.fetch_sub(1, std::memory_order_acq_rel);
    ...
    void ensure_recovery_workers_() {
        ...
        for (int w = 0; w < nw; ++w) {
            helper_threads_.emplace_back(&ConcordNative::helper_worker_loop_, this);
            decoder_threads_.emplace_back(&ConcordNative::decoder_worker_loop_, this);
            decoder_send_threads_.emplace_back(&ConcordNative::decoder_send_worker_loop_, this);
            failed_threads_.emplace_back(&ConcordNative::failed_worker_loop_, this);
        }
    ...
    py::dict get_recovery_batch_milestones(uint64_t batch_id) {
        RecoveryBatchState state;
        {
            ...
            if (!it->second.closed || it->second.done < it->second.expected)
                throw std::runtime_error("Concord: recovery batch is not complete");
            state = it->second;
            recovery_batches_.erase(it);
        }
        const uint64_t now_us = concord_now_us();
        py::dict result;
...
private:
    ...
    void init_ibv_() {
        if (ibv_fork_init() != 0)
            std::cerr << "[Concord RDMA] WARNING: ibv_fork_init() failed" << std::endl;

        int ndev;
        ibv_device** devs = ibv_get_device_list(&ndev);
        if (!devs || ndev == 0)
            throw std::runtime_error("Concord RDMA: no IB devices found");
        rdma_ctx_ = ibv_open_device(find_rdma_device_by_ip(
            my_ip_, devs, ndev, {"CONCORD"}));
        ibv_free_device_list(devs);
        if (!rdma_ctx_)
            throw std::runtime_error("Concord RDMA: failed to open device");

        rdma_pd_ = ibv_alloc_pd(rdma_ctx_);
        if (!rdma_pd_)
            throw std::runtime_error("Concord RDMA: failed to alloc PD");
    }
    ...
    static void configure_control_socket_(int fd) {
        ...
        if (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &enabled, sizeof(enabled)) != 0) {
            throw std::runtime_error(
                "Concord: failed to enable TCP_NODELAY: " + std::string(std::strerror(errno)));
        }
    }

    int create_tcp_connect(const std::string& ip, uint16_t port) {
        int fd = socket(AF_INET, SOCK_STREAM, 0);
        if (fd < 0) throw std::runtime_error("Concord: socket() failed");
        ...
        if (inet_pton(AF_INET, ip.c_str(), &addr.sin_addr) <= 0) {
            close(fd);
            throw std::runtime_error("Concord: invalid IP: " + ip);
        }
        ...
        while (true) {
            ...
            if (++attempt >= max_attempts) {
                int err = errno;
                close(fd);
                throw std::runtime_error("Concord: connect to " + ip + ":" +
        ...
        while (!acceptor_thread_stop_) {
            try {
                ...
                try {
                    ...
                    std::cerr << "[Concord RDMA] accept: " << e.what() << std::endl;
                    close(fd);
                    continue;
                }
                ...
                if (nr != (ssize_t)sizeof(hello)) {
                    std::cerr << "[Concord RDMA] accept: failed to recv connect hello" << std::endl;
                    close(fd);
                    continue;
                }
                {
                ...
                if (!acceptor_thread_stop_)
                    std::cerr << "[Concord RDMA] accept error: " << e.what() << std::endl;
                break;
            }
        }
    }
    ...
    // ---- Channel access ----
    ConcordRdmaChannel* get_channel_by_lane_(int peer_rg, int lane_id) {
    ...
    ConcordRdmaChannel* get_channel_(int peer_rg, int stripe_id) {
    ...
    ConcordRdmaChannel* get_save_send_channel_(int peer_rg, int logical_lane_id) {
        return get_channel_by_lane_(peer_rg, map_save_send_lane_(peer_rg, logical_lane_id));
    }

    ConcordRdmaChannel* get_save_recv_channel_(int peer_rg, int logical_lane_id) {
...
private:
    ...
    // Shared-lane tag layout: kind[63:60] | route[59:32] | generation[31:0].
    // Kinds used here are 0=source stripe, 1=deferred parity, 2=layer payload,
    // 3/4=recovery traffic selected by callers, and 5=aggregate parity.
    ...
    std::vector<std::vector<ConcordRdmaChannel*>> channels_;
    std::vector<std::unique_ptr<ConcordRdmaChannel>> channel_owners_;
    ...
    // ---- Shared RS encode worker pool state ----
...
PYBIND11_MODULE(concord_native, m) {
    py::class_<ConcordNative>(m, "ConcordNative")
        ...
        .def("path", &ConcordNative::path)
        .def("stop", &ConcordNative::stop)
        .def("is_stopped", &ConcordNative::is_stopped)

        // Stripe plan compilation (standalone, no RDMA needed)
        .def("compile_plans", &ConcordNative::compile_plans,
             py::arg("rank_in_group"))

        // RDMA init
        .def("init_rdma", &ConcordNative::init_rdma,
             py::arg("group_size"),
             py::arg("rank_in_group"),
             py::arg("base_port"),
             py::arg("my_ip"),
        ...
        .def("group_size", &ConcordNative::group_size)
        .def("rank_in_group", &ConcordNative::rank_in_group)

        // Buffer registration
        .def("register_buffer", &ConcordNative::register_buffer,
             py::arg("addr"), py::arg("size"))
        .def("register_recovery_buffer", &ConcordNative::register_recovery_buffer,
             py::arg("addr"), py::arg("size"))
        .def("unregister_buffer", &ConcordNative::unregister_buffer,
             py::arg("addr"))
        .def("clear_recovery_buffers", &ConcordNative::clear_recovery_buffers)

        // GDR capability
        .def_static("gdr_available", &ConcordNative::gdr_available)

        // Stripe decode (hardware recovery)
        .def("submit_stripe_decode_batch", &ConcordNative::submit_stripe_decode_batch,
             py::arg("k"),
             ...
             py::arg("block_size"))
        .def("submit_stripe_decode", &ConcordNative::submit_stripe_decode,
             py::arg("k"),
             py::arg("survivor_positions"),
        ...
        .def("send_to_peer", &ConcordNative::send_to_peer,
             py::arg("peer_rig"), py::arg("stripe_id"),
             py::arg("addr"), py::arg("size"),
             py::arg("batch_id") = 0, py::arg("tag_kind") = 3,
             py::call_guard<py::gil_scoped_release>())
        .def("recv_from_peer", &ConcordNative::recv_from_peer,
             py::arg("peer_rig"), py::arg("stripe_id"),
             py::arg("addr"), py::arg("size"),
             py::arg("batch_id") = 0, py::arg("tag_kind") = 3,
             py::call_guard<py::gil_scoped_release>())
        .def("send_layer_to_peer", &ConcordNative::send_layer_to_peer,
             py::arg("peer_rig"), py::arg("addr"), py::arg("size"), py::arg("batch_id"),
             py::arg("lane_id") = 0,
             py::call_guard<py::gil_scoped_release>())
        .def("send_layer_blocks_to_peer", &ConcordNative::send_layer_blocks_to_peer,
             py::arg("peer_rig"), py::arg("mirror_base"), py::arg("block_indices"),
             py::arg("block_size"), py::arg("batch_id"), py::arg("lane_id") = 0,
             py::call_guard<py::gil_scoped_release>())
        .def("get_max_send_sge", &ConcordNative::get_max_send_sge)
        .def("prepare_layer_recv_from_peer", &ConcordNative::prepare_layer_recv_from_peer,
             py::arg("peer_rig"), py::arg("addr"), py::arg("capacity"), py::arg("batch_id"),
             py::arg("lane_id") = 0)
        .def("wait_prepared_layer_recv_from_peer", &ConcordNative::wait_prepared_layer_recv_from_peer,
             py::arg("peer_rig"), py::arg("batch_id"), py::arg("lane_id") = 0,
             py::call_guard<py::gil_scoped_release>())
        .def("recv_layer_from_peer", &ConcordNative::recv_layer_from_peer,
             py::arg("peer_rig"), py::arg("addr"), py::arg("capacity"), py::arg("batch_id"),
             py::arg("lane_id") = 0,
             py::call_guard<py::gil_scoped_release>())
        .def("simple_exchange_with_peer", &ConcordNative::simple_exchange_with_peer,
             py::arg("peer_rig"), py::arg("send_addr"), py::arg("recv_addr"),
             py::arg("total_size"), py::arg("lanes"), py::arg("send_batch_id"),
             py::arg("recv_batch_id"),
             py::call_guard<py::gil_scoped_release>())
        .def("set_require_registered_mr", &ConcordNative::set_require_registered_mr,
             py::arg("require"))
        .def("set_debug", &ConcordNative::set_debug,
             py::arg("debug"))
        ...
        .def("encode_layer_stripes", &ConcordNative::encode_layer_stripes,
             ...
             py::call_guard<py::gil_scoped_release>())
        .def("encode_layer_stripes_batch", &ConcordNative::encode_layer_stripes_batch,
             py::arg("stripe_ids"), py::arg("data_addrs"), py::arg("p1_addrs"),
             py::arg("p2_addrs"), py::arg("block_sizes"),
             py::call_guard<py::gil_scoped_release>())
        .def("reset_ft_timing_stats", &ConcordNative::reset_ft_timing_stats)
        .def("get_ft_timing_stats", &ConcordNative::get_ft_timing_stats,
             "Return per-rank Concord timing counters including mirror D2H busy time")

        // Hardware recovery batch pipeline
        .def("init_recovery_plans", &ConcordNative::init_recovery_plans,
             py::arg("failed_nodes_1based"))
        .def("reset_recovery_generation", &ConcordNative::reset_recovery_generation,
             "Start a drained recovery generation while preserving plans and registered buffers")
        .def("begin_recovery_batch", &ConcordNative::begin_recovery_batch,
             py::arg("low_priority") = false)
        .def("end_recovery_batch", &ConcordNative::end_recovery_batch,
             py::arg("batch_id"))
        .def("wait_recovery_batch_id", &ConcordNative::wait_recovery_batch_id,
             py::arg("batch_id"), py::call_guard<py::gil_scoped_release>())
        .def("get_recovery_batch_milestones", &ConcordNative::get_recovery_batch_milestones,
             py::arg("batch_id"),
             "Return and release exact milestones for a completed recovery batch")
        .def("get_recovery_batch_timing_stats", &ConcordNative::get_recovery_batch_timing_stats,
             "Return timing counters for the most recently completed recovery batch")
        .def("reset_recovery_batch", &ConcordNative::reset_recovery_batch)
        .def("submit_recovery_stripe", &ConcordNative::submit_recovery_stripe,
             py::arg("stripe_id"),
             py::arg("block_size"),
             ...
             py::arg("active") = true)
        .def("submit_recovery_stripe_to_batch", &ConcordNative::submit_recovery_stripe_to_batch,
             py::arg("batch_id"),
             py::arg("stripe_id"),
             py::arg("block_size"),
             py::arg("helper_block_addr"),
        ...
        .def("submit_recovery_sentinel", &ConcordNative::submit_recovery_sentinel)
        .def("wait_recovery_batch", &ConcordNative::wait_recovery_batch,
             py::call_guard<py::gil_scoped_release>())
        .def("stop_recovery_runtime", &ConcordNative::stop_recovery_runtime,
             py::call_guard<py::gil_scoped_release>())
        .def("cleanup_recovery_runtime", &ConcordNative::cleanup_recovery_runtime,
             py::call_guard<py::gil_scoped_release>())

        // StripePlan queries
        .def("get_role_for_stripe", &ConcordNative::get_role_for_stripe,
             py::arg("stripe_id"))
        .def("get_source_node_ids", &ConcordNative::get_source_node_ids,
             py::arg("stripe_id"))
        ...
        .def("reset_layer", &ConcordNative::reset_layer)
        .def("reset_encode_layer", &ConcordNative::reset_encode_layer)
        .def("submit_source", &ConcordNative::submit_source,
             py::arg("stripe_id"), py::arg("data_addr"), py::arg("mirror_addr"), py::arg("block_size"))
        .def("submit_source_with_batch", &ConcordNative::submit_source_with_batch,
             py::arg("stripe_id"), py::arg("data_addr"), py::arg("mirror_addr"),
             py::arg("block_size"), py::arg("batch_id"))
        .def("skip_source_batch", &ConcordNative::skip_source_batch,
             py::arg("stripe_id"), py::arg("batch_id"))
        .def("submit_enc_recv", &ConcordNative::submit_enc_recv,
        ...
        .def("submit_enc_recv_with_batch", &ConcordNative::submit_enc_recv_with_batch,
             py::arg("stripe_id"), py::arg("recv_addr"), py::arg("p1_addr"),
             py::arg("p2_addr"), py::arg("block_size"),
             py::arg("source_active_mask"), py::arg("batch_id"))
        .def("submit_parity", &ConcordNative::submit_parity,
             py::arg("stripe_id"), py::arg("parity_in_addr"), py::arg("block_size"))
        // Legacy two-parity names retained for Python callers.
        .def("submit_p1_send", &ConcordNative::submit_p1_send,
             py::arg("stripe_id"), py::arg("p1_addr"), py::arg("block_size"))
        .def("submit_p2_send", &ConcordNative::submit_p2_send,
             py::arg("stripe_id"), py::arg("p2_addr"), py::arg("block_size"))
        .def("submit_async_p2_batch", &ConcordNative::submit_async_deferred_parity_batch,
             py::arg("parity_tasks"), py::arg("p2_send_tasks"))
        .def("submit_async_p2_layer", &ConcordNative::submit_async_deferred_parity_layer,
             py::arg("p2_addrs"), py::arg("block_size"))
        .def("submit_async_p2_layer_with_batch", &ConcordNative::submit_async_deferred_parity_layer_with_batch,
        ...
        .def("get_p2_route", &ConcordNative::get_deferred_parity_route, py::arg("stripe_id"))
        .def("submit_aggregate_p2", &ConcordNative::submit_aggregate_parity,
             py::arg("send_tasks"), py::arg("recv_tasks"), py::arg("generation"))
        .def("wait_aggregate_p2", &ConcordNative::wait_aggregate_parity,
             py::arg("timeout_seconds") = 300, py::call_guard<py::gil_scoped_release>())
        .def("reset_async_parity", &ConcordNative::reset_async_parity)
        .def("wait_layer", &ConcordNative::wait_layer,
             py::call_guard<py::gil_scoped_release>())
        .def("wait_encode_only", &ConcordNative::wait_encode_only,
             py::call_guard<py::gil_scoped_release>())
        .def("wait_parity_flush", &ConcordNative::wait_parity_flush,
             py::call_guard<py::gil_scoped_release>())
        .def("inc_pause_async_p2p", &ConcordNative::inc_pause_async_p2p)
        .def("dec_pause_async_p2p", &ConcordNative::dec_pause_async_p2p)
        .def("start_mirror_worker", &ConcordNative::start_mirror_worker)
        .def("wait_mirror_completion", &ConcordNative::wait_mirror_completion);
}
