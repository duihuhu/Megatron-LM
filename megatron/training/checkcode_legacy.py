"""CheckCode legacy checkpoint save/load (file renamed from eclatin_legacy.py).

Partial rename: CLI/help use CheckCode (--use-checkcode); internal symbols
(use_eclatin, ECLATINManager) and on-disk files (eclatin_main_rank*.pt,
MAGIC_ECLATIN) are unchanged for checkpoint compatibility.
"""
...
from megatron.core.dist_checkpointing.strategies.checkcode_manager import ECLATINManager
...
def _encode_eclatin_with_native(
    ...
    if native is None:
        raise RuntimeError("CheckCode native module is not initialized")

    buffers = manager.get_eclatin_buffers()
    if buffers is None:
        raise RuntimeError(
            "CheckCode legacy save: buffer pools are not initialized "
            "(disable --use-checkcode-layerwise for legacy CheckCode save)"
        )
    ...
    def get_free_data_buffer():
        ...
        try:
            ...
            logger.error("CheckCode legacy: timeout waiting for free data buffer")
    ...
    def get_free_recv_buffer():
        ...
        try:
            ...
            logger.error("CheckCode legacy: timeout waiting for free recv buffer")
    ...
    logger.info(
        f"CheckCode legacy: processing {total_bytes / (1024**3):.2f} GB "
    ...
    try:
        while src_pos < half_total:
            ...
            if data_block_1_offset_aligned + take > block_size:
                logger.warning("CheckCode legacy: data_block_1 exhausted")
                break
            if data_block_2_offset_aligned + take > block_size:
                logger.warning("CheckCode legacy: data_block_2 exhausted")
                break
            ...
            if parity_block_1_offset_aligned + take > block_size:
                logger.warning("CheckCode legacy: parity_block_1 exhausted")
                break
            if parity_block_2_offset_aligned + take > block_size:
                logger.warning("CheckCode legacy: parity_block_2 exhausted")
                break
...
def save_eclatin_legacy_checkpoint(state_dict: Dict[str, Any], checkpoint_name: str) -> None:
    ...
    if getattr(args, "use_eclatin_layerwise", False):
        raise RuntimeError(
            "CheckCode legacy save does not support --use-checkcode-layerwise; "
    ...
    if manager._eclatin_native is None:
        raise RuntimeError("CheckCode native module is not available in legacy save path")
    ...
    logger.info(f"CheckCode save timing: decompose {time.time()-t0:.3f}s")
    ...
    with torch.cuda.stream(d2h_stream):
        for i, (info, tensor) in enumerate(zip(decomposed.tensor_infos, decomposed.tensor_data)):
            ...
            if tensor_view.numel() != tensor_bytes:
                raise RuntimeError(
                    f"CheckCode legacy save: tensor bytes mismatch for {info.key}, "
    ...
    logger.info(f"CheckCode save timing: D2H+copy {time.time()-t0:.3f}s")

    t0 = time.time()
    rank_metadata, _ = _build_global_registry(local_tensor_metadata, {})
    logger.info(f"CheckCode save timing: metadata exchange {time.time()-t0:.3f}s")

    t0 = time.time()
    blocks = _allocate_eclatin_blocks_legacy(manager, rank_metadata)
    logger.info(f"CheckCode save timing: block alloc {time.time()-t0:.3f}s")

    t0 = time.time()
    if manager.use_rdma:
        manager.register_buffer(tensor_buffer)
    logger.info(f"CheckCode save timing: RDMA reg {time.time()-t0:.3f}s")
    ...
    logger.info(f"CheckCode save timing: encode {time.time()-t0:.3f}s")

    torch.distributed.barrier()
    logger.info(f"CheckCode legacy save: done in {time.time() - start_time:.2f}s")
...
def _load_eclatin_main_payload(checkpoint_dir: Path, rank: int, world_size: int) -> Dict[str, Any]:
    ...
    if world_size <= 1 or not torch.distributed.is_initialized():
        if local_payload is None:
            raise FileNotFoundError(f"CheckCode legacy: missing main file {main_path}")
    ...
    for r in range(world_size):
        if gathered[r] is not None:
            ...
            if all_ti and rank in all_ti:
                logger.info(
                    f"CheckCode legacy: eclatin_main_rank{rank}.pt missing locally; "
            ...
            if "tensor_infos" in payload:
                logger.warning(
                    f"CheckCode legacy: using rank {r}'s tensor_infos as fallback "
    ...
    raise FileNotFoundError(
        f"CheckCode legacy: eclatin_main_rank{rank}.pt missing on all ranks "
...
def _copy_eclatin_block_file_into_tensor(
    ...
    if not block_path.is_file():
        raise FileNotFoundError(f"CheckCode legacy load: missing block file {block_path}")
    from megatron.training.legacy_io_utils import (
...
def _load_eclatin_blocks_from_disk_into(
    ...
    if no_shared_block:
        if rank_in_group in (0, 1):
            ...
            raise RuntimeError(
                f"CheckCode legacy load: unexpected rank_in_group={rank_in_group}"
            )
        return
...
def _init_onefail_load_connections(
    ...
    logger.info(
        f"CheckCode legacy load: set load mode (failed_rank={failed_rank}, "
    ...
    if no_shared:
        ...
        if rank_in_group == 2:
            logger.info(
                "CheckCode legacy load: rank_in_group 2 init no-shared load accept connections"
            )
        ...
        if rank_in_group != 2:
            logger.info(
                f"CheckCode legacy load: rank_in_group {rank_in_group} connecting "
        ...
        if rank_in_group == 2:
            logger.info("CheckCode legacy load: rank_in_group 2 init load accept connections")
        ...
        if rank_in_group != 2:
            logger.info(
                f"CheckCode legacy load: rank_in_group {rank_in_group} connecting load send sockets"
            )
            native.init_load_connections(
                rank_in_group,
                receiver_ip,
    ...
    torch.distributed.barrier()
    logger.info("CheckCode one-fail: Phase 0 - all load connections established")
...
def _run_eclatin_full_recovery(
    ...
    if buffers is None:
        raise RuntimeError(
            "CheckCode one-fail: buffer pools are not initialized "
            "(disable --use-checkcode-layerwise for legacy CheckCode load)"
        )
    ...
    def get_free_recv_buffer() -> int:
        ...
        try:
            ...
            logger.error("CheckCode one-fail: timeout waiting for free recv buffer")
            return free_recv_queue.get()

    logger.info(
        f"CheckCode one-fail: starting chunked load recovery (rig{rank_in_group}), "
        f"no_shared_block={no_shared}, "
        ...
        while processed < aligned_half_block_size:
            ...
            if rank_in_group == 2:
                if recovered_buffer is None:
                    raise RuntimeError(
                        "CheckCode one-fail: rank_in_group 2 needs recovered_buffer"
                    )
                ...
                raise RuntimeError(
                    f"CheckCode one-fail: unexpected rank_in_group={rank_in_group}"
                )
    ...
    logger.info(
        f"CheckCode legacy: one-fail chunked pipeline done in {t_network_encode:.2f}s "
...
def _init_twofail_connections(
    ...
    logger.info("CheckCode two-failures: set load mode (failed_rank=10)")
    ...
    logger.info(f"CheckCode two-failures: Phase 0 - establishing connections (rig{rank_in_group})")
    ...
    torch.distributed.barrier()
    logger.info("CheckCode two-failures: Phase 0a - all listeners ready")
    ...
    native.wait_for_load_connections(timeout_seconds=30)
    torch.distributed.barrier()
    logger.info("CheckCode two-failures: Phase 0 - all connections established")
...
def _twofail_recv_block_addrs_at_offset(
    eclatin_blocks: Dict[str, Any], rank_in_group: int, offset: int
) -> List[int]:
    """Return recv addrs at byte offset within each eclatin block."""
    return [addr + offset for addr in _twofail_recv_block_addrs(eclatin_blocks, rank_in_group)]
...
def _run_eclatin_two_failures_recovery(
    ...
    """Chunked two-failures recovery via C++ pipeline (connections established in Phase 0)."""
    from time import time
    ...
    if buffers is None:
        raise RuntimeError(
            "CheckCode two-failures: buffer pools are not initialized "
            "(disable --use-checkcode-layerwise for legacy CheckCode load)"
        )
    ...
    def get_free_data_buffer() -> int:
        if poll_and_release is not None:
            poll_and_release()
        try:
            return free_data_queue.get(timeout=5.0)
        except queue.Empty:
            logger.error("CheckCode two-failures: timeout waiting for free data buffer")
            return free_data_queue.get()

    logger.info(
        f"CheckCode two-failures: starting chunked pipeline recovery (rig{rank_in_group}), "
        f"half_block={aligned_half_block_size / (1024**2):.0f} MB, "
        f"chunk={chunk_size / (1024**2):.0f} MB"
    )

    torch.distributed.barrier()
    native.reset_twofail_pipeline()
    ...
    try:
        while processed < aligned_half_block_size:
            ...
            if rank_in_group in (2, 3):
                peer_d1 = get_free_data_buffer()
                peer_d2 = get_free_data_buffer()
                out1 = get_free_data_buffer()
                out2 = get_free_data_buffer()
                out3 = get_free_data_buffer()
                out4 = get_free_data_buffer()

                own_d1 = int(eclatin_blocks["data_block_1"].data_ptr()) + off
                own_d2 = int(eclatin_blocks["data_block_2"].data_ptr()) + off
                own_p1 = int(eclatin_blocks["parity_block_1"].data_ptr()) + off
                own_p2 = int(eclatin_blocks["parity_block_2"].data_ptr()) + off

                native.submit_twofail_survivor_chunk(
                    rank_in_group,
                    own_d1, own_d2, own_p1, own_p2,
                    peer_d1, peer_d2,
                    out1, out2, out3, out4,
                    take, chunk_index,
                    peer_d1, peer_d2, out1, out2, out3, out4,
                )
            elif rank_in_group in (0, 1):
                recv_addrs = _twofail_recv_block_addrs_at_offset(
                    eclatin_blocks, rank_in_group, off
                )
                native.submit_twofail_failed_chunk(
                    rank_in_group,
                    recv_addrs[0], recv_addrs[1], recv_addrs[2], recv_addrs[3],
                    take, chunk_index,
                )
    ...
    native.submit_twofail_pipeline_sentinels()
    native.wait_for_twofail_pipeline_completion()
    ...
    logger.info(
        f"CheckCode legacy: two-failure chunked pipeline done in {t_network_encode:.2f}s "
        f"({chunk_index} chunks)"
    )
    return t_network_encode
...
def load_eclatin_legacy_checkpoint(checkpoint_name: str) -> Dict[str, Any]:
    """
    Load CheckCode torch legacy checkpoint: run recovery (aligned with torch_dist), then reconstruct
    ...
    if not getattr(args, "use_eclatin", False):
        logger.warning(
            "CheckCode legacy load: args.use_eclatin is False; enabling for native module init"
        )
    ...
    manager.init_eclatin_if_enabled()
    if manager._eclatin_native is None:
        raise RuntimeError("CheckCode native module is not available in legacy load path")
    ...
    if two_failures:
        if rank_in_group in (0, 1):
            ...
            raise RuntimeError(
                f"CheckCode two-failures: unexpected rank_in_group={rank_in_group}"
            )
    ...
    if two_failures:
        ...
        _mode = "HW"
    logger.info(
        "CheckCode legacy load timing (%s): network_encode=%.2fs",
    ...
    logger.info(f"CheckCode legacy load: cleaning up native module (rank {rank})")
    manager.cleanup()
