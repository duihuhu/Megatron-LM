...
"""LEGACY checkpoint path for Concord (POA-driven stripe encode with RDMA).
...
def _canonical_concord_model_key(key: str) -> str:
...
_CONCORD_INFO_PROFILE_EVENT = "recovery_parity_repair_done"

_concord_first_layer_recovery_start_s: Optional[float] = None
_concord_first_layer_recovery_target_idx: Optional[int] = None
_concord_first_layer_milestones: Dict[str, Any] = {}
_concord_first_layer_milestones_lock = threading.Lock()
_concord_first_layer_cuda_start_events: Dict[int, Any] = {}
_concord_first_layer_pending_cuda_events: Dict[str, Dict[str, Any]] = {}
_concord_first_layer_generation: int = 0
_concord_first_layer_failed_rank: bool = False
_concord_parity_timing_lock = threading.Lock()
_concord_parity_timing: Dict[str, Any] = {}


_concord_async_runtime_timing_reported: bool = False


def _reset_concord_parity_completion_timing() -> None:
    global _concord_parity_timing
...
def concord_record_first_microbatch_done(done_s: Optional[float] = None) -> None:
    ...
    with _concord_parity_timing_lock:
        ...
        if int(_concord_parity_timing.get("generation", -1)) != int(
            ...
            return
        if float(_concord_parity_timing.get("first_microbatch_done_s", 0.0)) <= 0.0:
            _concord_parity_timing["first_microbatch_done_s"] = now
        _stash_concord_parity_completion_timing_locked()


def _record_concord_parity_done() -> None:
    now = time.time()
    with _concord_parity_timing_lock:
        if not _concord_parity_timing:
            return
        if int(_concord_parity_timing.get("generation", -1)) != int(
            _concord_first_layer_generation
        ):
            return
        _concord_parity_timing["parity_done_s"] = now
        _stash_concord_parity_completion_timing_locked()
...
def _start_concord_first_layer_recovery_timer(target_layer_idx: Optional[int] = None) -> None:
    global _concord_first_layer_recovery_start_s, _concord_first_layer_recovery_target_idx
    global _concord_first_layer_milestones, _concord_first_layer_generation
    global _concord_first_layer_failed_rank
    rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
    failed_rank = rank in _get_concord_failed_ranks()
    _reset_concord_parity_completion_timing()
    with _concord_first_layer_milestones_lock:
        _concord_first_layer_generation += 1
        _concord_first_layer_recovery_start_s = None
        _concord_first_layer_recovery_target_idx = target_layer_idx
        _concord_first_layer_failed_rank = failed_rank
        _concord_first_layer_cuda_start_events.clear()
        _concord_first_layer_pending_cuda_events.clear()
        _concord_first_layer_milestones = {
...
def _arm_concord_first_layer_cuda_timer() -> None:
    ...
    if torch.cuda.is_available():
        try:
            ...
            device_idx = -1
    with _concord_first_layer_milestones_lock:
        _concord_first_layer_recovery_start_s = timer_start_s
        generation = _concord_first_layer_generation
        failed_rank = _concord_first_layer_failed_rank
        if start_event is not None:
            _concord_first_layer_cuda_start_events[device_idx] = start_event
    rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
    with _concord_parity_timing_lock:
        _concord_parity_timing = {
...
def _record_concord_first_layer_milestone(
    ...
    with _concord_first_layer_milestones_lock:
        ...
        if name not in _concord_first_layer_milestones:
            return
        if _concord_first_layer_milestones.get(valid_key, False):
            elapsed_s = max(float(_concord_first_layer_milestones[name]), elapsed_s)
        _concord_first_layer_milestones[name] = float(elapsed_s)
        _concord_first_layer_milestones[valid_key] = True
        _stash_concord_first_layer_milestones_locked()
...
def _stash_concord_first_layer_cuda_event(
    ...
    if layer_idx != _concord_first_layer_recovery_target_idx:
        return
    if not _concord_first_layer_failed_rank:
        return
    with _concord_first_layer_milestones_lock:
        if (
            layer_idx != _concord_first_layer_recovery_target_idx
            or _concord_first_layer_milestones.get(f"{name}_valid", False)
            or name in _concord_first_layer_pending_cuda_events
        ):
            return
        if generation != _concord_first_layer_generation:
            return
        _concord_first_layer_pending_cuda_events[name] = {
            "event": event,
...
def _record_concord_first_layer_cuda_event(name: str, layer_idx: int) -> None:
    if layer_idx != _concord_first_layer_recovery_target_idx:
        return
    if not _concord_first_layer_failed_rank:
        return
    if not torch.cuda.is_available():
        timer_start_s = _concord_first_layer_recovery_start_s
        if timer_start_s is not None:
            _record_concord_first_layer_milestone(
    ...
    with _concord_first_layer_milestones_lock:
        if generation != _concord_first_layer_generation:
            return
        _concord_first_layer_pending_cuda_events.setdefault(
            name,
            {
                "event": event,
                "device": device_idx,
...
def resolve_concord_first_layer_cuda_events() -> None:
    if _concord_first_layer_recovery_start_s is None:
        return
    with _concord_first_layer_milestones_lock:
        pending = dict(_concord_first_layer_pending_cuda_events)
        start_events = dict(_concord_first_layer_cuda_start_events)
        generation = _concord_first_layer_generation
    resolved = {}
    ...
    with _concord_first_layer_milestones_lock:
        ...
        for name, (layer_idx, elapsed_s) in resolved.items():
            ...
            _concord_first_layer_milestones[name] = float(elapsed_s)
            _concord_first_layer_milestones[valid_key] = True
            _concord_first_layer_pending_cuda_events.pop(name, None)
        _stash_concord_first_layer_milestones_locked()
...
def _get_concord_failed_ranks() -> Set[int]:
    try:
        ...
        return set()
    configured = getattr(args, "concord_failed_ranks_parsed", None)
    if configured is None:
        configured = getattr(args, "concord_failed_ranks", None)
    if configured is None:
        return set()
...
def _concord_recovery_profile(role: str, event: str, **fields) -> None:
    ...
    if event.endswith("_error"):
        logger.warning("Concord profile: %s", " ".join(parts))
    elif event == _CONCORD_INFO_PROFILE_EVENT and rank == 0:
        logger.info("Concord profile: %s", " ".join(parts))
    else:
        logger.debug("Concord profile: %s", " ".join(parts))


def _concord_recovery_role(
...
def _resolve_optimizer_distribute_target(
    ...
    if not model_key:
        return None
    canonical_key = _canonical_concord_model_key(model_key)
...
def _build_optimizer_model_key_map(state_dict: Dict[str, Any]) -> Dict[str, str]:
    ...
    for model_key, _layer_idx, optimizer_keys in entries:
        canonical_model_key = _canonical_concord_model_key(model_key)
        for opt_key in optimizer_keys:
    ...
    def finish(self) -> None:
        with self._lock:
            ...
            self.error = RuntimeError(
                "Concord ordered recovery finished without required common tensors: "
    ...
    def wait_snapshot(self) -> Dict[str, torch.Tensor]:
        ...
        with self._lock:
            if self.error is not None:
                raise RuntimeError("Concord common-state recovery failed") from self.error
            if self.snapshot is None:
                missing = sorted(self.required_keys.difference(self._views))
                raise RuntimeError(
                    "Concord common-state recovery completed without required tensors: "
                    f"missing_count={len(missing)} keys={missing[:8]}"
                )
...
@dataclass
class _ConcordLayerRecoveryJob:
    encode_iter: int
    layer_name: str
    layer_idx: int
    ...
    actual_size: int
    layer_infos: List
    target_actual_sizes: Dict[int, int]
    member_actual_sizes: Dict[int, int]


@dataclass
class _ConcordRecoveryWindow:
...
@dataclass
class _ConcordRecoveredWindow:
    window: _ConcordRecoveryWindow
...
class _ConcordLayerwiseRuntime:
    """Concord layer-ready registry consumed by TransformerBlock."""

    def __init__(self, records: List[_ConcordLayerReadyRecord]) -> None:
        self._records_by_layer: Dict[int, _ConcordLayerReadyRecord] = {
            r.layer_idx: r for r in records if r.layer_idx >= 0
        }
        self._records_by_name: Dict[str, _ConcordLayerReadyRecord] = {
    ...
    def _layer_model_tensors(
        self, record: _ConcordLayerReadyRecord
    ...
    def _release_injected_record_tensors(self, record: _ConcordLayerReadyRecord) -> None:
    ...
    def _try_prefetch_layer_locked(self, layer_idx: int) -> bool:
        ...
        t0 = time.time()
        with _concord_first_layer_milestones_lock:
            milestone_generation = _concord_first_layer_generation
        try:
    ...
    def wait_and_materialize_layer(self, layer_idx: int) -> bool:
        ...
        try:
            ...
            if first_touch and _concord_debug_enabled():
                logger.debug(
        ...
        try:
            for layer_idx, record in sorted(self._records_by_layer.items()):
                ...
                if not record.optimizer_ready:
                    raise RuntimeError(
                        f"Concord optimizer layer {record.layer_name} is not ready: "
                        f"{record.error}"
                    )
        finally:
    ...
    def resolve_model_h2d_timing(self) -> None:
        ...
        recovery_starts = {}
        with _concord_first_layer_milestones_lock:
            recovery_starts = dict(_concord_first_layer_cuda_start_events)
        span_s = 0.0
        done_s = 0.0
...
class _ConcordRecoveryService:
    """Single owner for Concord recovery state across load and training safe points."""
    ...
    def __init__(self) -> None:
        ...
        self.runtime: Optional[_ConcordLayerwiseRuntime] = None
    ...
    def reset_for_load(self, role: str) -> None:
        ...
        if old_prepare_thread is not None:
            if old_prepare_thread.is_alive():
                raise RuntimeError(
                    "Concord cannot reset recovery while the previous optimizer "
                    "CPU prepare thread is still running"
                )
            if _optimizer_prepare_error is not None:
                raise RuntimeError(
                    "Concord previous optimizer CPU prepare failed before reset"
                ) from _optimizer_prepare_error
        self.role = role
        _concord_async_runtime_timing_reported = False
        self.runtime = None
        self.worker = None
    ...
    def _check_worker_error(self) -> None:
        ...
        if worker is None:
            return
        error_holder = getattr(worker, "_concord_error_holder", None)
    ...
    def wait_all(self, reason: str = "explicit") -> None:
        ...
        if worker is not None and worker.is_alive():
            ...
            jobs = getattr(worker, "_concord_job_count", -1)
            _concord_recovery_profile(role, "join_wait_start", jobs=jobs, reason=reason)
            t_join = time.time()
...
_active_layerwise_runtime: Optional[_ConcordLayerwiseRuntime] = None
_active_recovery_worker: Optional[threading.Thread] = None
_active_recovery_service: Optional[_ConcordRecoveryService] = None
...
def _get_active_concord_recovery_service() -> _ConcordRecoveryService:
    global _active_recovery_service
    if _active_recovery_service is None:
        _active_recovery_service = _ConcordRecoveryService()
...
def _set_active_concord_recovery_worker(worker: Optional[threading.Thread]) -> None:
    global _active_recovery_worker
    _active_recovery_worker = worker
    _get_active_concord_recovery_service().attach_worker(worker)
...
def _make_pending_layerwise_records(
    ...
    all_concord_dirs: List[Optional[Path]],
    checkpoint_dir: Path,
    saved_block_size: int,
    n_encode_iters: int,
    recovery_rank: Optional[int] = None,
) -> List[_ConcordLayerReadyRecord]:
...
def install_concord_layerwise_runtime_from_state_dict(
    ...
    global _active_layerwise_runtime
    metadata = state_dict.pop("__concord_layerwise_runtime__", None)
    if not metadata:
        _set_active_concord_layerwise_runtime(None)
        return
    ...
    runtime = _ConcordLayerwiseRuntime(records)
    model_sd = state_dict.get("model")
    if isinstance(model_sd, dict):
        runtime.attach_model_state_keys(model_sd.keys())
    if model is not None:
    ...
    if _concord_debug_enabled():
        logger.debug(
            "Concord layerwise runtime installed: ready_layers=%d live_tensors=%d "
...
def concord_filter_layerwise_model_placeholders(
    ...
    if removed and _concord_debug_enabled():
        logger.debug(
...
def concord_wait_and_materialize_layer(layer_idx: int) -> bool:
    """Wait for a recovered Concord layer to be ready before forward."""
    return _get_active_concord_recovery_service().wait_layer(layer_idx)


def concord_async_layerwise_active() -> bool:
...
def concord_materialize_all_layers() -> None:
    ...
    concord_wait_for_async_recovery()


def concord_materialize_first_layer_for_timer() -> bool:
...
def concord_restore_optimizer_control_state(optimizer=None) -> bool:
    ...
    if len(optimizers) == 1:
        ...
        raise RuntimeError(
            "Concord optimizer control restore cannot map checkpoint state to "
            f"{len(optimizers)} chained optimizers"
        )
    if len(states) != len(optimizers):
        raise RuntimeError(
            "Concord optimizer control restore count mismatch: "
    ...
    for index, (optim_instance, optim_state) in enumerate(zip(optimizers, states)):
        ...
        if current_device is None:
            raise RuntimeError(
                "Concord optimizer control restore requires CUDA for grad scaler state"
            )
        ...
        if not torch.is_tensor(scale) or scale.device.type != "cuda":
            raise RuntimeError(
                "Concord grad scaler restore did not produce a CUDA scale tensor: "
                f"optimizer_index={index} scale={scale!r}"
            )
        if int(scale.device.index) != int(current_device):
            raise RuntimeError(
                "Concord grad scaler restored on the wrong CUDA device: "
                f"optimizer_index={index} expected=cuda:{current_device} "
                f"actual={scale.device}"
            )
        restored += 1

    if restored and _concord_debug_enabled():
        rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
        logger.debug(
...
def _concord_optimizer_state_dict(optim_state: Dict[str, Any]) -> Optional[Dict[Any, Any]]:
...
def _concord_optimizer_uses_cpu_offload(optimizer) -> bool:
...
def _sync_concord_model_params_to_optimizer_main_params(optimizer) -> bool:
...
def _concord_hw_optimizer_overlap_enabled() -> bool:
    try:
...
def _finish_deferred_optimizer_state(runtime, service) -> None:
    ...
    if (
        not _concord_recovery_async_parity_enabled()
...
def _prepare_deferred_optimizer_state(optimizer, runtime, service):
    if runtime is None:
        ...
        cpu_offload = _concord_optimizer_uses_cpu_offload(optimizer)
        prepare_s = 0.0
        if cpu_offload:
    ...
    clone_recovery_tensors = _concord_optimizer_uses_cpu_offload(optimizer)
    t_prepare = time.time()
    ...
    concord_normalize_optimizer_state_param_keys(_pending_optimizer_state)
    if updated != expected:
        raise RuntimeError(
            "Concord optimizer recovery: incomplete deferred optimizer "
    ...
    if missing_fp32_keys and _concord_debug_enabled():
        logger.debug(
    ...
    if not has_checkpoint_fp32_master:
        if _concord_hw_optimizer_overlap_enabled():
            raise RuntimeError(
                "Concord optimizer overlap requires complete recovered FP32 master "
        ...
        _sync_concord_model_params_to_optimizer_main_params(optimizer)
...
def concord_discard_pending_optimizer_state() -> bool:
    ...
    if _pending_optimizer_state is None and _pending_optimizer_container is None:
        return False
    if _get_active_concord_recovery_service().runtime is not None:
        raise RuntimeError(
            "Concord cannot discard pending optimizer state on a failed recovery rank"
        )
    _pending_optimizer_container = None
    _pending_optimizer_state = None
    return True
...
def concord_log_optimizer_overlap_not_started() -> None:
...
def concord_prepare_optimizer_h2d(optimizer=None) -> bool:
    ...
    if _optimizer_h2d_prepared or _pending_optimizer_state is None or optimizer is None:
        return False
    if not _concord_hw_optimizer_overlap_enabled():
        return False
    service = _get_active_concord_recovery_service()
    runtime = service.runtime
    if runtime is None:
...
def concord_start_optimizer_h2d(optimizer=None) -> bool:
...
def concord_wait_optimizer_h2d(optimizer=None) -> bool:
...
def concord_wait_for_optimizer_state(optimizer=None) -> bool:
...
def _log_concord_async_runtime_timing_once(context: str) -> None:
    global _concord_async_runtime_timing_reported
    if _concord_async_runtime_timing_reported:
        return
    ...
    rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
    _concord_async_runtime_timing_reported = True
    if rank == 0:
        logger.debug(
            "Concord async runtime timing (%s): layer_inject_s=%.6fs "
...
def finalize_concord_recovery_timing() -> None:
    ...
    runtime = _get_active_concord_recovery_service().runtime
    if runtime is not None:
...
def concord_recovery_safe_point(point: str) -> None:
    ...
    if not getattr(args, "use_concord", False):
        return
    service = _get_active_concord_recovery_service()
    _concord_recovery_profile(service.role, "safe_point", point=point, state=service.state)
...
def _group_by_layer(
    ...
    if distribute_common and -1 in groups and len(groups) > 1:
        ...
        if debug:
            mapped_optimizer = sum(
                ...
                if _classify_concord_tensor(info.key, optimizer_layer_map).kind == "optimizer_layer"
            )
    ...
    if debug:
        logger.info(
            "Concord save grouping: distribute_common=%s moved_model_common=%d tensors "
        ...
        if optimizer_layer_keys or optimizer_common_keys:
            logger.info(
                "Concord save optimizer key summary: layer=%s common=%s",
...
def _synchronize_layer_groups(layer_groups: List[_LayerGroup]) -> List[_LayerGroup]:
    """Ensure all ranks in an Concord group iterate the same layer indexes."""
    ...
    if added_empty and _concord_debug_enabled():
        logger.info(
            "Concord save grouping: rank=%d added %d empty layer groups for "
...
def _exchange_concord_group_metadata(
...
def _gather_all_concord_dirs(
    ...
    if world_size > 1 and torch.distributed.is_initialized():
        ...
        return [_path_from_gathered_entry(p, "concord dir gather") for p in gathered]
    return [Path(my_dir)]
...
def _prepare_layer_exchange_network(
    ...
    if not chunks and block_size > 0:
        raise RuntimeError("Concord layer exchange produced no chunks")
    ...
    def _task_id(sid: int, chunk_idx: int) -> int:
        ...
        if batch_id >= (1 << 12) or sid >= (1 << 10) or chunk_idx >= (1 << 10):
            raise RuntimeError(
                "Concord layer exchange task id fields exceed deterministic tag layout"
            )
    ...
    if payload_bytes > int(layer_gpu.numel()) or payload_bytes > int(layer_mirror_cpu.numel()):
        raise RuntimeError(
            f"Concord layer exchange: canonical mirror too small for layer {layer_idx}"
        )
    ...
    if exchange_chunk_bytes == 0:
        send_seg = max(1, int(os.environ.get("CONCORD_LAYER_EXCHANGE_SEG", "1")))
...
def wait_for_concord_parity_flush() -> None:
    """Drain Concord save-side parity work before in-process recovery."""
    _wait_previous_async_writers(debug=_concord_debug_enabled())
    native = ConcordManager().get_native()
...
def _wait_async_p2_writer(
    ...
    if _async_writer_error is not None:
        error = _async_writer_error
        _async_writer_error = None
        raise RuntimeError("Concord asynchronous P2 writer failed") from error
    context = descriptor.get("completed_context")
    if context is None:
        raise RuntimeError("Concord asynchronous P2 writer produced no completed context")
    return context
...
def save_concord_legacy_checkpoint(
    state_dict: Dict[str, Any], checkpoint_name: str, write_to_disk: bool = True
) -> None:
    """Write concord_main_rank*.pt + layer-wise source/parity shards."""
    t0 = time.time()
    rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
    ...
    concord_gdr = bool(getattr(args, "concord_gdr", False))
    ...
    if _dbg:
        logger.info(f"CONCORD save timing: decompose+group {time.time()-t0:.3f}s")
    ...
    # 2. Init Concord manager + RDMA + stripe plans (once, shared across layers)
    manager = ConcordManager()
    manager.init_concord_if_enabled()
    native = manager.get_native()
    if native is None:
        raise RuntimeError("Concord legacy save: native module not available")
    ...
    if native.group_size() != native.n():
        raise RuntimeError(
            f"Concord legacy save: group_size={native.group_size()} != n={native.n()}"
        )
    ...
    layer_groups = _group_by_layer(
        decomposed,
        distribute_common=getattr(args, "concord_distribute_common", False),
        debug=_dbg,
    ...
    if _dbg:
        logger.info(f"CONCORD save timing: layer group+manager init {time.time()-t0:.3f}s")
    ...
    trace_save = _dbg or os.environ.get("CONCORD_TRACE_INIT", "0") == "1"
    if trace_save:
        logger.info("CONCORD save trace rank=%d: layer buffers already registered", rank)

    if world_size > 1:
        if trace_save:
            logger.info("CONCORD save trace rank=%d: before pre-encode barrier", rank)
        torch.distributed.barrier()
        if trace_save:
            logger.info("CONCORD save trace rank=%d: after pre-encode barrier", rank)
    e2e_t0 = time.time()
    ...
    if exchange_chunk_mb < 0:
        raise RuntimeError("CONCORD_LAYER_EXCHANGE_CHUNK_MB must be non-negative")
    ...
    if use_layer_exchange_encode:
        ...
        if int(chunk_min.item()) != int(chunk_max.item()):
            raise RuntimeError(
                "CONCORD_LAYER_EXCHANGE_CHUNK_MB must match across all distributed ranks"
            )
        if int(lanes_min.item()) != int(lanes_max.item()):
            raise RuntimeError(
                "CONCORD_LAYER_EXCHANGE_LANES must match across all distributed ranks"
            )
        if int(order_min.item()) != int(order_max.item()):
            raise RuntimeError(
                "CONCORD_LAYER_FRONTIER_ORDER must match across all distributed ranks"
            )
    ...
    if use_layer_exchange_encode:
        ...
        def _wait_layer_chunks_ready(
            ...
            if missing:
                raise RuntimeError(f"Concord layer mirror chunk events missing: {missing[:4]}")
            wait_t0 = time.time()
            for key in chunks:
        ...
        coalesce_us = max(
            0, int(os.environ.get("CONCORD_LAYER_ENCODE_COALESCE_US", default_coalesce_us))
        )
        ...
        def _encode_coordinator() -> None:
            ...
            def _finish_batch(
                ...
                if error is not None:
                    raise RuntimeError("Concord native layer encode failed") from error
            ...
            if use_submit_worker:
                submit_thread = threading.Thread(
                    target=_submit_worker, name="concord-layer-encode-submit"
                )
                submit_thread.start()
            for ctx in layer_exchange_contexts:
            ...
            try:
                while True:
                    ...
                    if not batch:
                        ...
                        if incomplete_done:
                            ...
                            raise RuntimeError(
                                f"Concord layer exchange encode incomplete for {layer_name}: "
                ...
                for ctx in layer_exchange_contexts:
                    ...
                    if missing or reserved_keys[id(ctx)]:
                        raise RuntimeError(
                            f"Concord layer encode final validation failed: "
                ...
                if submit_thread is not None:
                    ...
                    if submit_thread.is_alive() and not encode_errors:
                        encode_errors.append(
                            RuntimeError("Concord encode submit worker failed to stop")
                        )
        ...
        if worker_errors:
            raise RuntimeError("Concord layer exchange worker failed") from worker_errors[0]
        if encode_errors:
            raise RuntimeError("Concord layer exchange encode failed") from encode_errors[0]
    ...
    if use_cross_layer_encode:
        ...
        if _dbg or trace_save:
            logger.info(
                "CONCORD save: wait_encode_only for cross-layer batch layers=%d",
                len(encode_results),
            )
        native.wait_encode_only()
    ...
    if not _use_async_parity and hasattr(native, "start_mirror_worker"):
        native.start_mirror_worker()
    ...
    if has_native_timing:
        ...
        if mirror_failed or mirror_completed != mirror_submitted:
            raise RuntimeError(
                f"Concord mirror incomplete: completed={mirror_completed} "
    ...
    if rank == 0:
        ...
        logger.info(
            "CONCORD save layer-exchange transport (%(mode)s): "
        ...
        if has_native_timing:
            ...
            if _dbg or trace_save:
                ...
                logger.info(
                    "CONCORD save submit detail (%(mode)s): queue_setup_s=%(lx_queue_setup_s).4fs "
...
def _assert_concord_main_metadata_only(main_path: Path) -> None:
    ...
    if is_raw_format(str(main_path), MAGIC_CONCORD):
        with open(main_path, "rb") as f:
            ...
            if magic != MAGIC_CONCORD:
                return
            _, _, data_len = _read_header(f)
            if data_len > 0:
                raise RuntimeError(
                    f"Concord load: main file {main_path} has data_len={data_len} "
                    "(legacy payload format). Re-save with metadata-only main + CNBK blocks."
                )
        return
    ...
    if torch.is_tensor(tb) and tb.numel() > 0:
        raise RuntimeError(
            f"Concord load: main file {main_path} contains tensor_buffer "
            "(legacy format). Re-save with metadata-only main + CNBK blocks."
        )
...
def _read_local_concord_main_payload(
    ...
    if not main_path.is_file():
        raise FileNotFoundError(f"Concord legacy: missing main file {main_path}")
    if load_tensor_buffer:
        raise RuntimeError(
            "Concord load: load_tensor_buffer=True is no longer supported; "
            "assemble from local CNBK SOURCE blocks instead."
        )
...
def _recovery_common_wave_size(n: int) -> int:
    ...
    try:
        return max(1, min(int(configured), max_concurrency))
    except ValueError as exc:
        raise RuntimeError(
            "Concord recovery: CONCORD_RECOVERY_CONCURRENCY must be an integer"
        ) from exc
...
def _allocate_recovery_buf_pool(
    ...
    if concurrency_override is not None:
        concurrency = max(1, min(int(concurrency_override), max_concurrency))
    else:
        concurrency = _recovery_common_wave_size(n)
    decoder_recv_buf_slots: List[List[torch.Tensor]] = []
...
def _make_layer_recovery_job(
    ...
    saved_block_size: int,
    recovery_rank: Optional[int] = None,
    failed_group_ranks: Optional[List[int]] = None,
    group_members: Optional[List[int]] = None,
) -> Optional[_ConcordLayerRecoveryJob]:
    """Build one group-synchronized recovery iteration with rank-local schema."""
    metadata_rank = rank
    participating_ranks = list(group_members or [metadata_rank])
    metadata_by_rank: Dict[int, Tuple[str, Dict[str, Any]]] = {}
    for member in participating_ranks:
        member_order = all_layer_order.get(member)
        if member_order is None:
            if member == metadata_rank:
                member_order = list(main_payload.get("layer_names", []))
            else:
                raise RuntimeError(
                    "Concord recovery: missing saved layer order for group member "
                    f"rank={member} encode_iter={encode_iter}"
                )
        if encode_iter >= len(member_order):
            raise RuntimeError(
                "Concord recovery: group member is missing synchronized encode iteration "
                f"rank={member} encode_iter={encode_iter} order_len={len(member_order)}"
            )
        member_layer_name = member_order[encode_iter]
        ...
        if member_meta is None:
            raise RuntimeError(
                ...
                f"rank={member} encode_iter={encode_iter} layer={member_layer_name}"
            )
        metadata_by_rank[member] = (member_layer_name, member_meta)

    block_sizes = {
        int(meta.get("block_size", saved_block_size) or saved_block_size)
        for _layer_name, meta in metadata_by_rank.values()
    }
    if len(block_sizes) != 1:
        details = {
            member: (layer_name, int(meta.get("block_size", saved_block_size) or saved_block_size))
            for member, (layer_name, meta) in metadata_by_rank.items()
        }
        raise RuntimeError(
            "Concord recovery: inconsistent saved block_size at synchronized "
            f"encode_iter={encode_iter}: {details}"
        )
    layer_block_size = next(iter(block_sizes))
    layer_name, meta = metadata_by_rank[metadata_rank]
    ...
    member_actual_sizes = {
        ...
        for member, (_member_layer_name, meta) in metadata_by_rank.items()
    }
    target_actual_sizes: Dict[int, int] = {}
    for failed_rank in failed_group_ranks or []:
        failed_node = member_to_node.get(int(failed_rank))
    ...
    if layer_name.startswith("layer_"):
        try:
            ...
            layer_idx = -1
    return _ConcordLayerRecoveryJob(
        encode_iter=encode_iter,
        layer_name=layer_name,
        layer_idx=layer_idx,
        ...
        target_actual_sizes=target_actual_sizes,
        member_actual_sizes=member_actual_sizes,
    )
...
def _materialize_recovered_layer(
    ...
    global_tensor_infos: List,
    runtime: Optional[_ConcordLayerwiseRuntime] = None,
...
def _build_failed_source_block_indices(
    ...
    for plan in plans:
        ...
        if not active_by_stripe.get(sid, True):
            continue
        targets = plan.get('failed_targets', []) if plan.get('dual_failure') else [plan]
        for target in targets:
            ...
            if blk_idx < 0:
                raise RuntimeError(
                    ...
                    f"stripe={sid} node={failed_node}"
                )
            source_indices[(sid, failed_node)] = blk_idx
...
def _iter_recovery_windows_for_job(
    ...
    raw_data_plans, parity_plans, active_by_stripe, skipped_padding_stripes, n_filled_blocks = (
        _build_recovery_layer_context(manager, n, job, job.actual_size, job.layer_block_size)
    )
...
def _concord_recovery_parity_repair_enabled() -> bool:
...
def _concord_recovery_async_parity_enabled() -> bool:
    """Return whether recovery parity repair runs behind the load thread."""
    if not _concord_recovery_parity_repair_enabled():
        return False
    try:
        from megatron.training import get_args
...
def _ensure_recovery_parity_layer_buffers(
    ...
    for job in jobs:
        ...
        if group_members:
            for rig, _global_rank in enumerate(group_members):
...
def _validate_recovery_alignment(
    ...
    if encode_iters != list(range(len(jobs))):
        raise RuntimeError(
            "Concord recovery: jobs must contain contiguous encode_iter values; "
            f"got {encode_iters}"
        )
    for job in jobs:
        if int(job.layer_block_size) <= 0:
            raise RuntimeError(
                "Concord recovery: invalid layer block size "
                f"encode_iter={job.encode_iter} size={job.layer_block_size}"
            )
    stripe_ids = [int(plan.get("stripe_id", -1)) for plan in plans]
    if stripe_ids != sorted(stripe_ids) or len(stripe_ids) != len(set(stripe_ids)):
        raise RuntimeError(
...
def _prepare_recovery_parity_repair_buffers(
    ...
    for plan in decoder_plans + helper_plans:
        ...
        if blk is None:
            ...
            raise RuntimeError(
                "Concord recovery parity repair: missing preloaded block "
...
def _persist_recovered_parity_wave(
    ...
    my_node = int(manager.rank_in_group) + 1
    written = 0
    for relative_slot, plan in enumerate(plans):
        slot = slot_start + relative_slot
        if not _is_failed_in_recovery_plan(plan, my_node):
            continue
        role = _recovery_original_role_for_node(plan, my_node)
        if role not in (int(StripeRole.ENCODER), int(StripeRole.PARITY_TARGET)):
            continue
        ...
        try:
            with open(temp_path, "wb") as file:
                file.write(header)
                file.write(memoryview(block.contiguous().numpy()))
                file.flush()
...
def _submit_recovery_parity_repair(
    ...
    max_inflight = _recovery_common_wave_size(n)
    if buf_pool is not None and buf_pool.concurrency != max_inflight:
        raise RuntimeError(
            "Concord recovery parity: buffer concurrency differs from common wave size "
            f"({buf_pool.concurrency} != {max_inflight})"
        )
    t_network = time.time()
    waves = 0
    for start in range(0, len(parity_plans), max_inflight):
        ...
        if hasattr(native, "get_recovery_batch_milestones"):
            # Milestone retrieval erases native batch state. Keep it alive through
            # persistence so completion/error state has a single lifetime boundary.
            native.get_recovery_batch_milestones(batch_id)
        waves += 1
...
def _set_pending_recovery_parity_repair(
    ...
    if not jobs or not _concord_recovery_parity_repair_enabled():
        return
...
def _run_recovery_parity_repair_submissions(pending: Dict[str, Any], reason: str) -> None:
    ...
    try:
        for job in jobs:
            ...
            total_persisted_shards += int(timing.get("persisted_shards", 0.0))
            _concord_recovery_profile(
                recovery_role,
        ...
        if hasattr(native, "get_recovery_batch_timing_stats"):
            ...
            pause_wait_s = max(
                0.0,
                float(native_timing.get("low_priority_pause_wait_s", 0.0) or 0.0)
                - pause_wait_start_s,
            )
...
def _start_recovery_parity_repair_submissions(reason: str, role: str = "unknown") -> None:
    ...
    worker = threading.Thread(
        ...
        args=(pending, reason),
        name=f"concord-recovery-parity-rank{pending['rank']}",
        daemon=False,
    )
    ...
    try:
        ...
        _recovery_async_parity_thread = None
        _pending_recovery_parity_repair = pending
        raise
...
def _finish_recovery_parity_repair_submissions(reason: str, role: str = "unknown") -> None:
    ...
    if worker is not None:
        t_join = time.time()
        _concord_recovery_profile(role, "recovery_parity_join_start", reason=reason)
        worker.join()
        _recovery_async_parity_thread = None
        _concord_recovery_profile(
    ...
    if _recovery_async_parity_error is not None:
        ...
        _recovery_async_parity_error = None
        raise RuntimeError("Concord recovery parity repair failed") from exc


def _wait_recovery_data_before_parity(reason: str) -> _ConcordRecoveryService:
...
def _concord_recovery_lifecycle_needs_finish(
    service: Optional[_ConcordRecoveryService] = None,
) -> bool:
    """Return whether this process owns recovery resources that still need cleanup."""
    if service is None:
        service = _get_active_concord_recovery_service()
    return bool(
        _pending_recovery_parity_repair is not None
        or _recovery_async_parity_thread is not None
        or _recovery_async_parity_error is not None
        or service.state not in (service.INIT, service.TORN_DOWN)
    )
...
def _dispatch_recovery_parity_after_data_recovery(
    ...
    if _concord_recovery_async_parity_enabled():
        _start_recovery_parity_repair_submissions("after_data_recovery", role)
    else:
        _run_pending_recovery_parity_repair_sync("after_data_recovery", role)


def concord_start_or_run_recovery_parity_after_forward_backward() -> bool:
    """Fallback-dispatch parity if the data-completion path did not start it."""
    service = _get_active_concord_recovery_service()
    service._check_worker_error()
    if service.worker is None and _pending_recovery_parity_repair is not None:
        _dispatch_recovery_parity_after_data_recovery(service.role)
...
def _run_pending_recovery_parity_repair_sync(
    ...
    if pending is None:
        return
    _pending_recovery_parity_repair = None
    pending["execution_mode"] = "sync_data_worker"
    _run_recovery_parity_repair_submissions(pending, reason)
    # There is no worker to join in sync mode, but finish propagates and clears
    # any recorded repair error without flushing or tearing down recovery state.
...
def _run_recovery_pipeline(
...
) -> List[Tuple[Optional[_ConcordLayerReadyRecord], Dict[str, float]]]:
    t_pipeline = time.time()
    completed: "queue.Queue[Optional[_ConcordRecoveredWindow]]" = queue.Queue(maxsize=2)
    results: List[Tuple[Optional[_ConcordLayerReadyRecord], Dict[str, float]]] = []
    ...
    def _network_worker() -> None:
        ...
        def _mark_window_done(recovered: _ConcordRecoveredWindow) -> None:
        ...
        try:
            pending_windows: List[Tuple[_ConcordRecoveryWindow, Dict[int, torch.Tensor]]] = []
            job_windows: Dict[int, List[_ConcordRecoveryWindow]] = {}
            ...
            max_inflight_stripes = _recovery_common_wave_size(n)
            if buf_pool is not None and buf_pool.concurrency != max_inflight_stripes:
                raise RuntimeError(
                    "Concord recovery: buffer concurrency differs from common wave size "
                    f"({buf_pool.concurrency} != {max_inflight_stripes})"
                )
            free_ranges: List[Tuple[int, int]] = [(0, max_inflight_stripes)]
            active_windows: List[Tuple[_ConcordRecoveredWindow, int, int]] = []
            idx = 0
            while idx < len(pending_windows):
                ...
                if required_slots > max_inflight_stripes:
                    raise RuntimeError(
                        "Concord recovery: one POA recovery window requires "
...
def _start_layer_recovery_worker(
    ...
    setattr(worker, "_concord_job_count", len(jobs))
    worker.start()
    return worker
...
def _normalize_stripe_block(
    ...
    if blk is None or blk.numel() == 0:
        raise RuntimeError(
            "Concord recovery: required stripe block is missing or empty "
            f"rank={rank} layer={layer_name} stripe={stripe_id}"
        )
...
def _preload_recovery_stripe_blocks(
    ...
    stripe_plans = ConcordManager().stripe_plans
    ...
    def _load_one(job: Tuple[int, Dict]) -> Tuple[int, int, int, torch.Tensor]:
        ...
        if encode_iter >= len(my_order):
            raise RuntimeError(
                ...
                f"rank={rank} encode_iter={encode_iter} order_len={len(my_order)}"
            )
    ...
    if _concord_debug_enabled():
        logger.debug(
            "Concord recovery preload rank=%d: source=%d encoder_p1=%d parity_p2=%d "
...
def _prep_survivor_hw_disk(
    ...
    if preload_recovery and recovery_stripe_plans:
        ...
        for encode_iter in range(n_encode_iters):
            ...
            for plan in participating:
                sid = int(plan['stripe_id'])
                role = _stripe_role_for_node(manager, sid, my_node, manager.concord_n)
                if role == int(StripeRole.SOURCE):
...
def _build_recovery_layer_context(
    ...
    enable_padding_skip = os.environ.get("CONCORD_RECOVERY_SKIP_PADDING", "0") == "1"
...
def _prepare_recovery_layer_buffers(
    ...
    for plan in decoder_stripes + helper_stripes:
        ...
        blk = preloaded_blocks.get(sid)
        if blk is None:
            role = _stripe_role_for_node(manager, int(sid), my_node, n)
            raise RuntimeError(
                "Concord recovery: missing active preloaded block "
                f"layer_idx={layer_idx} stripe={sid} role={role} node={my_node}"
            )
        my_blocks[sid] = blk
    ...
    for submit_idx, stri_plan in enumerate(window.plans):
        ...
        if _is_failed_in_recovery_plan(stri_plan, my_node):
            ...
            if active and window.layer_buf is not None and original_role == int(StripeRole.SOURCE):
                ...
                if blk_idx is None:
                    raise RuntimeError(
                        "Concord recovery: missing source block index for "
...
def _submit_recovery_network(
    ...
    if is_failed:
        ...
        if n_stored != expected_stored:
            raise RuntimeError(
                f"Concord recovery: layer {layer_name} stored {n_stored} "
...
def _teardown_concord_after_training() -> None:
    ...
    if not getattr(args, "use_concord", False):
        return
    rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
    debug = _concord_debug_enabled()
    if debug:
        logger.debug("CONCORD teardown trace rank=%d: enter Concord teardown", rank)
    service = _get_active_concord_recovery_service()
    manager = ConcordManager()
    if service.safe_point_teardown_done and manager.get_native() is None:
        if debug:
            logger.debug("CONCORD teardown trace rank=%d: teardown already completed", rank)
        return
    if debug:
        logger.debug("CONCORD teardown trace rank=%d: before recovery worker wait", rank)
    concord_wait_for_async_recovery()
    concord_drain_recovery_parity("after_training", allow_start_pending=True)
    if debug:
        logger.debug("CONCORD teardown trace rank=%d: after recovery worker wait", rank)
    if manager.get_native() is None:
        return
    if debug:
        logger.debug("Concord: tearing down native module after training (rank %d)", rank)
    _wait_previous_async_writers(debug=debug, rank=rank)
    if debug:
        logger.debug("CONCORD teardown trace rank=%d: save writers drained", rank)
    if debug:
        logger.debug("CONCORD teardown trace rank=%d: before manager cleanup", rank)
    manager.cleanup(teardown=True)
    if debug:
        logger.debug("CONCORD teardown trace rank=%d: after manager cleanup", rank)
...
def _teardown_concord_native_after_load() -> None:
    ...
    if manager.get_native() is None:
        return
    rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
    role = getattr(manager, "_concord_recovery_role", "unknown")
    service = _get_active_concord_recovery_service()
    ...
    if inprocess_reuse:
        ...
        _concord_recovery_profile(role, "teardown_skipped_inprocess_reuse")
        if _concord_debug_enabled():
            logger.debug(
                "Concord legacy load: keeping native module alive for in-process reuse (rank %d)",
                rank,
            )
        return
...
def _concord_checkpoint_file_signature(
    checkpoint_dir: Path, failed_global_ranks: List[int]
) -> Tuple[Tuple[str, int, int], ...]:
    """Stat immutable Concord inputs; parity-repair outputs are excluded."""
    root = checkpoint_dir.resolve()
    entries: List[Tuple[str, int, int]] = []
    for dirpath, dirnames, filenames in os.walk(root):
        dirnames.sort()
        for filename in sorted(filenames):
            if not (filename.startswith("concord_") or filename.endswith(".pt")):
                continue
            # Recovery parity repair intentionally rewrites failed-rank shards.
            # They are outputs, not preload inputs, and must not invalidate the cache.
            if filename.startswith("concord_shard_rank"):
                match = re.match(r"concord_shard_rank(\d+)(?:_|\.)", filename)
                if match and int(match.group(1)) in failed_global_ranks:
                    continue
...
def _concord_inprocess_workspace_key(
...
def _prepare_concord_inprocess_workspace_reset(role: str) -> float:
    """Drain the previous generation before cached buffers can be overwritten."""
    start = time.time()
    service = _get_active_concord_recovery_service()
    ...
    if _pending_optimizer_state is not None or _pending_optimizer_container is not None:
        raise RuntimeError(
            ...
            "state still references the previous recovery generation."
        )
    return time.time() - start


def _log_concord_inprocess_setup_timing(cache_status: str, values: Dict[str, float]) -> None:
    maxima = _timing_max_dict(values)
    rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
    if rank == 0:
        logger.info(
...
def recover_concord_legacy_hardware(
    ...
    2. network_encode — per-layer RDMA RS data recovery only; parity is profiled separately
    ...
    native = manager.get_native()
    if native is None:
        raise RuntimeError("Concord hardware recovery: native module not available")
    ...
    workspace_enabled = _concord_inprocess_workspace_enabled()
    workspace_key = None
    workspace = None
    ...
    if workspace_enabled:
        setup_timing["reset_s"] = _prepare_concord_inprocess_workspace_reset(
            getattr(manager, "_concord_recovery_role", "unknown")
        )
        workspace_key = _concord_inprocess_workspace_key(
            checkpoint_dir, manager, failed_global_ranks
        )
        workspace = manager.get_inprocess_recovery_workspace(workspace_key)
    cache_hit = workspace is not None
    metadata_plan_start = time.time()
    ...
    if workspace is None:
        ...
        all_tensor_infos = workspace["all_tensor_infos"]
    ...
    if _dbg:
        logger.debug(
            "Concord recovery: main loaded encode_iters=%d group_layer_orders=%s",
            n_encode_iters,
            {r: all_layer_order.get(r, []) for r in group_members},
        )

    if workspace is None:
        all_concord_dirs = _gather_all_concord_dirs(checkpoint_dir, world_size)
    else:
    ...
    for failed_rank in failed_global_ranks:
        failed_group = manager._get_group_id(failed_rank, world_size, n)
    ...
    layerwise_records: List[_ConcordLayerReadyRecord] = []
    ...
    if async_forward and is_failed:
        ...
        _set_active_concord_layerwise_runtime(runtime_for_recovery)
        if _dbg:
            logger.debug(
                "Concord recovery: async-forward runtime prepared layers=%d "
                "(worker-backed recovery joins before state_dict rebuild)",
                len(pending_records),
            )

    if workspace is not None:
        recovery_jobs = workspace["recovery_jobs"]
    else:
        recovery_jobs: List[_ConcordLayerRecoveryJob] = []
        for encode_iter in range(n_encode_iters):
            if not involved:
                continue
            job = _make_layer_recovery_job(
    ...
    setup_timing["alloc_touch_register_s"] = time.time() - alloc_start
    if workspace_enabled and workspace is None:
        workspace = {
            "main_payload": copy.copy(main_payload),
            "all_tensor_infos": all_tensor_infos,
            "all_layer_order": all_layer_order,
            "all_layer_metadata": all_layer_metadata,
            "all_concord_dirs": all_concord_dirs,
            "recovery_stripe_plans": manager.recovery_stripe_plans,
            "recovery_dual_failure": manager.recovery_dual_failure,
    ...
    def _accumulate_layer_timings(
        layer_results: List[Tuple[Optional[_ConcordLayerReadyRecord], Dict[str, float]]]
    ) -> None:
    ...
    if detached_transformer_recovery:
        ...
        _set_active_concord_recovery_worker(recovery_worker)
        if _dbg:
            logger.debug(
    ...
    safe_point = getattr(args, "concord_recovery_safe_point", "load")
    parity_pending = _pending_recovery_parity_repair is not None
    if safe_point == "load" and not detached_transformer_recovery and not parity_pending:
        _teardown_concord_native_after_load()
...
def _reconstruct_from_main_payload(main_payload: Dict, flat_key_roots: list = None) -> Dict[str, Any]:
    ...
    if tensor_buffer is None:
        raise RuntimeError(
            "Concord load: tensor_buffer missing in main payload; "
            "assemble from local CNBK SOURCE blocks before reconstruct."
        )
...
def _reconstruct_from_tensor_views(
    ...
    if missing_keys:
        raise RuntimeError(
            "Concord load: missing tensor views for direct reconstruct: "
...
def _reconstruct_common_from_tensor_views(
    ...
    if missing_keys:
        ...
        raise RuntimeError(
            "Concord load: missing common tensor views for async reconstruct: "
            f"actual_view_count={len(tensor_views_by_key)} "
...
def _compile_stripe_plans_for_load(
    ...
    from megatron.core.dist_checkpointing.strategies import concord_manager as _fm

    strategies_dir = os.path.dirname(os.path.abspath(_fm.__file__))
    so_files = glob.glob(os.path.join(strategies_dir, "concord_native*.so"))
    if not so_files:
        raise RuntimeError("Concord load: concord_native*.so not found")
    spec = importlib.util.spec_from_file_location("concord_native", so_files[0])
...
def _build_full_buf_from_local_blocks(
    ...
    """Assemble rank-local tensor bytes into full_buf from SOURCE CNBK shards."""
    ...
    layer_names = main_payload.get("layer_names", [])
    if not layer_names:
        raise RuntimeError("Concord load: layer_names empty in main metadata")
    ...
    if out_buf is not None:
        if out_buf.numel() < total_tensor_size + safety_margin:
            raise RuntimeError(
                f"Concord load: out_buf too small ({out_buf.numel()} "
...
def _assemble_state_dict_from_local_blocks(
    ...
    try:
        ...
        if getattr(args, "use_concord", False):
            ...
            set_ft_load_timing_context("CONCORD", "SW", timings)
    except Exception:
        pass
    if teardown_native:
        args = None
        try:
            ...
            safe_point = "load"
        inprocess_software_active = bool(
            args is not None
            and getattr(args, "ft_inprocess_recovery_benchmark", False)
            and getattr(args, "_ft_inprocess_recovery_active", False)
            and getattr(args, "ft_inprocess_recovery_software_failure", False)
        )
...
def load_concord_legacy_checkpoint(checkpoint_name: str) -> Dict[str, Any]:
    """Load Concord legacy checkpoint.

    Default: all ranks assemble tensor data from local SOURCE CNBK blocks;
    main files provide metadata only.

    Hardware recovery (--use-concord-hardware-failure): failed ranks recover
    ...
    failed_ranks_str = getattr(args, "concord_failed_ranks", None)
    failed_ranks_parsed = getattr(args, "concord_failed_ranks_parsed", None)
    if _concord_debug_enabled():
        logger.debug(
