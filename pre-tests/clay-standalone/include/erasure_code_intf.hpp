...
using bufferptr = ceph::bufferptr;
using bufferhash = ceph::bufferhash;

using ErasureCodeProfile = std::map<std::string, std::string>;

inline auto operator<<(std::ostream &out, const ErasureCodeProfile &profile)
    -> std::ostream & {

  // use stream_buf to ensure the sync output
  std::stringstream out_buf{};
  out_buf << "{";
  for (auto it = profile.begin(); it != profile.end(); ++it) {
    if (it != profile.begin())
      out_buf << ",";
    out_buf << it->first << "=" << it->second;
  }
  out_buf << "}";
  out << out_buf.rdbuf();
  return out;
...
public:
   ...
   */
  virtual auto init(ErasureCodeProfile &profile, std::ostream *ss) -> int = 0;
   ...
   */
  [[nodiscard]] virtual auto get_profile() const
      -> const ErasureCodeProfile & = 0;
   ...
   */
  [[nodiscard]] virtual auto get_chunk_count() const -> unsigned int = 0;
   ...
   */
  [[nodiscard]] virtual auto get_data_chunk_count() const -> unsigned int = 0;
   ...
   */
  [[nodiscard]] [[nodiscard]] virtual auto get_coding_chunk_count() const
      -> unsigned int = 0;
   ...
   */
  virtual auto get_sub_chunk_count() -> int = 0;
   ...
   */
  [[nodiscard]] virtual auto get_chunk_size(unsigned int object_size) const
      -> unsigned int = 0;
   ...
   * @return **0** on success or a negative errno on error.
   */
  virtual auto minimum_to_decode(
      const std::set<int> &want_to_read, const std::set<int> &available,
      std::map<int, std::vector<std::pair<int, int>>> *minimum) -> int = 0;
   ...
   * @return **0** on success or a negative errno on error.
   */
  virtual auto minimum_to_decode_with_cost(const std::set<int> &want_to_read,
                                           const std::map<int, int> &available,
                                           std::set<int> *minimum) -> int = 0;
   ...
   * @return **0** on success or a negative errno on error.
   */
  virtual auto encode(const std::set<int> &want_to_encode, const bufferlist &in,
                      std::map<int, bufferlist> *encoded) -> int = 0;

  virtual auto encode_chunks(const std::set<int> &want_to_encode,
                             std::map<int, bufferlist> *encoded) -> int = 0;
   ...
   * @return **0** on success or a negative errno on error.
   */
  virtual auto decode(const std::set<int> &want_to_read,
                      const std::map<int, bufferlist> &chunks,
                      std::map<int, bufferlist> *decoded, int chunk_size)
      -> int = 0;

  virtual auto decode_chunks(const std::set<int> &want_to_read,
                             const std::map<int, bufferlist> &chunks,
                             std::map<int, bufferlist> *decoded) -> int = 0;
   ...
   */
  [[nodiscard]] virtual auto get_chunk_mapping() const
   ...
   * @return **0** on success or a negative errno on error.
   */
  virtual auto decode_concat(const std::map<int, bufferlist> &chunks,
                             bufferlist *decoded) -> int = 0;
};
