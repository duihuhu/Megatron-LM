...
# Usage: ./test_eccheck_4nodes_node_335M.sh <node_rank> <gpu_index> [additional_args...]
# Example: ./test_eccheck_4nodes_node_335M.sh 0 1
...
# Set CUDA_VISIBLE_DEVICES according to GPU_INDEX argument
...
CHECKPOINT_PATH="/workspace/data/checkpoint/models/gpt2-345m-0-basic-ec"
DATA_PATH="/workspace/models/gpt2-345m-0/codeparrot_content_document"

SHM_PKT="/dev/shm/shm_pkt"

# Remaining args after node-rank/gpu-index are passed to the training script
ARGS_TO_PASS=("$@")
...
if [ "${PRINT_CMD:-0}" != "0" ]; then
    echo "Would run (Node $NODE_RANK, GPU $GPU_INDEX): PYTHONPATH=$PYTHONPATH:/workspace/Megatron-LM torchrun ${DISTRIBUTED_ARGS[@]} pretrain_gpt.py ${GPT_ARGS[@]} ${DATA_ARGS[@]} ${MODEL_PARALLEL_ARGS[@]} ${EVAL_AND_LOGGING_ARGS[@]} --distributed-backend nccl ${ARGS_TO_PASS[@]}"
    exit 0
fi
# -------------------------------------------------------------------------

echo "Starting Node $NODE_RANK with GPU $GPU_INDEX"
