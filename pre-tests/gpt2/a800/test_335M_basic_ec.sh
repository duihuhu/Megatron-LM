...
BASIC_EC_SINGLE_NODE_FAILED_RANKS=${BASIC_EC_FAILED_RANKS:-$(make_rank_range "$GPUS_PER_NODE")}
BASIC_EC_TWO_NODE_FAILED_RANKS=${BASIC_EC_HW2_FAILED_RANKS:-$(make_rank_range "$((2 * GPUS_PER_NODE))")}
...
CHECKPOINT_PATH="/dev/shm/data/checkpoint/models/gpt2-345m-0-basic-ec" #<Specify path>
...
case "$MODE" in
    ...
    hardware)
        RECOVERY_MODE_ARGS=(
            --load $CHECKPOINT_PATH
            --basic-ec-failed-ranks "$BASIC_EC_SINGLE_NODE_FAILED_RANKS"
            #--basic-ec-hw-debug
        )
        ;;
    hardware2)
        RECOVERY_MODE_ARGS=(
            --load $CHECKPOINT_PATH
            --basic-ec-failed-ranks "$BASIC_EC_TWO_NODE_FAILED_RANKS"
            --basic-ec-require-hw2
        )
        ;;
    inprocess_sw)
        RECOVERY_MODE_ARGS=(
        ...
        ;;
    inprocess)
        set_inprocess_recovery_args "$BASIC_EC_SINGLE_NODE_FAILED_RANKS"
        ;;
    inprocess2)
        set_inprocess_recovery_args "$BASIC_EC_TWO_NODE_FAILED_RANKS"
        RECOVERY_MODE_ARGS+=(--basic-ec-require-hw2)
        ;;
esac
...
EVAL_AND_LOGGING_ARGS=(
    ...
    #--use-basic-ec-software-failure
    #--basic-ec-failed-ranks 0   # Comma-separated failed global ranks for hardware recovery only; software recovery is set to 2
