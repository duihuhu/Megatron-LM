...
CHECKPOINT_PATH="/dev/shm/data/checkpoint/models/gpt2-345m-0-checkcode" #<Specify path>
DATA_PATH="/workspace/models/gpt2-345m-0/codeparrot_content_document" #<Specify path and file prefix>_text_document

SHM_PKT="/dev/shm/shm_pkt"
MODE=save
if [ -n "$1" ] && [[ "$1" =~ ^(save|software|hardware|hardware_noshared|hardware2)$ ]]; then
    MODE="$1"
    shift
fi
ARGS_TO_PASS=("$@")
...
EVAL_AND_LOGGING_ARGS=(
    ...
    #--use-checkcode-software-failure # toggle for software failure; untoggled for hardware failure
    #--use-checkcode-two-failures # toggle for two failures
    --eval-iters 1
