...
# Concord (POA-driven stripe encode with RDMA) multi-node script.
# Usage: ./test_335M_concord.sh <node_rank> <gpu_id_0> [gpu_id_1 ...] [mode] [additional_args...]
# Example: ./test_335M_concord.sh 0 0
# Example (2 GPUs per container): ./test_335M_concord.sh 0 2 3 inprocess2
export CONCORD_ENABLE_RECOVERY_PARITY_REPAIR=1
export CONCORD_RECOVERY_SKIP_PADDING=0
#export CONCORD_TRACE_INIT=1
...
export CONCORD_RDMA_LANES_PER_PEER=8
export CONCORD_ALLOW_UNSAFE_LANE_SHARING=1
export CONCORD_LAYER_EXCHANGE_SEG=4
export CONCORD_LAYER_ENCODE_BATCH=2
...
export CONCORD_BASE_IP=$MASTER_ADDR
# Concord RDMA listens on CONCORD_BASE_PORT + rank_in_group.
# Keep this separate from torchrun MASTER_PORT and move it if a port is busy.
export CONCORD_BASE_PORT=${CONCORD_BASE_PORT:-27200}
...
if [ "${#GPU_IDS[@]}" -eq 0 ]; then
    ...
    echo "Usage: ./test_335M_concord.sh <node_rank> <gpu_id_0> [gpu_id_1 ...] [mode] [additional_args...]"
    exit 1
fi
...
CONCORD_SINGLE_NODE_FAILED_RANKS=${CONCORD_FAILED_RANKS:-$(make_rank_range "$GPUS_PER_NODE")}
CONCORD_TWO_NODE_FAILED_RANKS=${CONCORD_HW2_FAILED_RANKS:-$(make_rank_range "$((2 * GPUS_PER_NODE))")}
...
CHECKPOINT_PATH="/dev/shm/data/checkpoint/models/gpt2-345m-0-concord"
...
case "$MODE" in
    save)
        ...
        if [ "${CONCORD_ASYNC_PARITY:-1}" != "0" ]; then
            RECOVERY_MODE_ARGS+=(--concord-async-parity)
        fi
        ;;
    software)
        RECOVERY_MODE_ARGS=(
    ...
    hardware)
        RECOVERY_MODE_ARGS=(
            ...
            --concord-async-recovery-forward
            --concord-failed-ranks "$CONCORD_SINGLE_NODE_FAILED_RANKS"
            --concord-recovery-safe-point optimizer_step
            --concord-recovery-only-teardown
    ...
    hardware2)
        RECOVERY_MODE_ARGS=(
            ...
            --use-concord-hardware-failure
            --concord-async-recovery-forward
            --concord-failed-ranks "$CONCORD_TWO_NODE_FAILED_RANKS"
            --concord-recovery-safe-point optimizer_step
            --concord-recovery-only-teardown
            # Native RDMA stays alive until optimizer_step; forked DataLoader workers segfault.
            --num-workers 0
    ...
    inprocess_sw)
        RECOVERY_MODE_ARGS=(
            ...
            --rerun-mode disabled
            --ft-inprocess-recovery-repeat $FT_INPROCESS_RECOVERY_REPEAT
            --ft-inprocess-recovery-failed-ranks "$CONCORD_SINGLE_NODE_FAILED_RANKS"
            --ft-inprocess-recovery-after-train-iter 0
            --ft-inprocess-recovery-exit-after-forward
        )
        ;;
    inprocess)
        set_inprocess_recovery_args "$CONCORD_SINGLE_NODE_FAILED_RANKS"
        ;;
    inprocess2)
        set_inprocess_recovery_args "$CONCORD_TWO_NODE_FAILED_RANKS"
        ;;
esac

if [[ "$MODE" =~ ^(hardware|hardware2|inprocess|inprocess2)$ ]] \
    && [ "${CONCORD_RECOVERY_ASYNC_PARITY:-1}" != "0" ]; then
    RECOVERY_MODE_ARGS+=(--concord-recovery-async-parity)
fi
...
EVAL_AND_LOGGING_ARGS=(
    ...
    --eval-interval 100
    #--save $CHECKPOINT_PATH
    --concord-skip-load-teardown-barrier
    #--concord-defer-load-teardown
    #--load $CHECKPOINT_PATH
    ...
    --use-concord
    --concord-distribute-common
    #--concord-debug
    --concord-n 8
    #--use-concord-software-failure
    #--concord-failed-ranks 0
...
echo "CONCORD_BASE_IP: $CONCORD_BASE_IP"
