...
"""Concord layer-exchange network-only microbenchmark.

This benchmark reuses ConcordNative RDMA layer send/recv APIs and constructs a
...
def _load_native():
    matches = glob.glob(str(STRATEGY_DIR / "concord_native*.so"))
    if not matches:
        raise RuntimeError(f"No concord_native*.so in {STRATEGY_DIR}")
    spec = importlib.util.spec_from_file_location("concord_native", matches[0])
...
def _get_ip(rank: int, local_rank: int) -> str:
    return resolve_ip("CONCORD", rank=rank, local_rank=local_rank)
...
def _run_exchange(native, send_tasks, recv_tasks) -> float:
    ...
    if errors:
        raise RuntimeError("Concord bench exchange failed") from errors[0]
    return elapsed


def main() -> None:
    parser = argparse.ArgumentParser(description="Concord layer-exchange network-only microbenchmark")
    ...
    parser.add_argument("--base-port", type=int, default=int(os.environ.get("CONCORD_BASE_PORT", "27200")))
    parser.add_argument("--n", type=int, default=8)
    parser.add_argument("--bytes", default="1G")
    parser.add_argument("--iters", type=int, default=20)
    parser.add_argument("--warmup", type=int, default=5)
    parser.add_argument("--lanes-per-peer", type=int, default=int(os.environ.get("CONCORD_RDMA_LANES_PER_PEER", "8")))
    parser.add_argument("--send-lanes-per-peer", type=int, default=int(os.environ.get("CONCORD_SEND_LANES_PER_PEER", "0")))
    parser.add_argument("--recv-lanes-per-peer", type=int, default=int(os.environ.get("CONCORD_RECV_LANES_PER_PEER", "0")))
    parser.add_argument("--segments", type=int, default=int(os.environ.get("CONCORD_LAYER_EXCHANGE_SEG", "4")))
    ...
    if args.world_size % args.n != 0:
        raise RuntimeError("Concord bench requires world_size to be divisible by n")
    if args.n < 2:
        raise RuntimeError("Concord bench requires n >= 2")
    if not args.simple and args.n < 3:
        raise RuntimeError("Concord normal mode requires n >= 3; use --simple for n=2")
    ...
    if group_base_port > 65000:
        raise RuntimeError(
            f"Concord RDMA group_base_port={group_base_port} exceeds uint16 limit; lower --base-port"
        )
    ...
    if forward_lanes <= 0 or reverse_lanes <= 0:
        raise RuntimeError("Concord split lanes require positive send and recv lane counts")
    ...
    os.environ["CONCORD_RDMA_LANES_PER_PEER"] = str(total_lanes)
    if total_lanes < args.n * (args.n - 1):
        os.environ.setdefault("CONCORD_ALLOW_UNSAFE_LANE_SHARING", "1")
    ...
    native = native_mod.ConcordNative(args.n)
    ...
    for it in range(total_iters):
        ...
        if it >= args.warmup:
            ...
            print(
                f"ITER_RESULT,mode=concord,rank={args.rank},group_id={group_id},"
    ...
    print(
        f"RESULT,mode=concord,rank={args.rank},group_id={group_id},rank_in_group={rank_in_group},"
        f"concord_n={args.n},simple={int(args.simple)},cpu_send={int(args.cpu_send)},iters={args.iters},"
