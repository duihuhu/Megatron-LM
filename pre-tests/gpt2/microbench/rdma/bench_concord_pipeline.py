#!/usr/bin/env python3
"""Temporary Concord CPU-RDMA, CUDA D2H, and ISA-L overlap microbenchmark."""
import argparse
import os
import threading
import time
...
from bench_concord_network import (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Temporary Concord pipeline overlap microbenchmark")
    ...
    parser.add_argument("--base-port", type=int, default=int(os.environ.get("CONCORD_BASE_PORT", "28200")))
    ...
    parser.add_argument("--lanes-per-peer", type=int, default=int(os.environ.get("CONCORD_RDMA_LANES_PER_PEER", "4")))
    ...
    if args.n != 4:
        raise RuntimeError("This benchmark models Concord n=4")
    ...
    try:
        ...
        os.environ["CONCORD_RDMA_LANES_PER_PEER"] = str(lanes * 2)
        if lanes * 2 < args.n * (args.n - 1):
            os.environ.setdefault("CONCORD_ALLOW_UNSAFE_LANE_SHARING", "1")
        native = _load_native().ConcordNative(args.n)
        ...
        for mode in modes:
            ...
            if args.rank == 0:
                csv_line("RESULT", {
                    ...
                    "cluster_nodes": args.cluster_nodes, "concord_n": args.n,
                    ...
                    "mode_order": args.mode_order, "cpu_list": os.environ.get("CONCORD_RS_CPU_LIST", "unset"),
