...
NETWORK_BENCH="$SCRIPT_DIR/bench_concord_network.py"
...
CONCORD_BASE_PORT=${CONCORD_BASE_PORT:-27200}
...
run_encode() {
    build_encode
    echo "Running ISA-L Concord batch encode: workers=$CPU_WORKERS cpus=$CPU_LIST total=$CPU_TOTAL jobs=$CPU_JOBS"
...
check_network_node() {
    ...
    compgen -G "$STRATEGY_DIR/concord_native*.so" >/dev/null || {
        echo "ERROR,no concord_native*.so in $STRATEGY_DIR" >&2
        return 1
    }
    local hca
...
export_network_env() {
    ...
    export CONCORD_LOCAL_RANK_NIC_0=mlx5_0
    export CONCORD_LOCAL_RANK_NIC_1=mlx5_0
    export CONCORD_LOCAL_RANK_NIC_2=mlx5_1
    export CONCORD_LOCAL_RANK_NIC_3=mlx5_1
    export CONCORD_LOCAL_RANK_NIC_4=mlx5_4
    export CONCORD_LOCAL_RANK_NIC_5=mlx5_4
    export CONCORD_LOCAL_RANK_NIC_6=mlx5_5
    export CONCORD_LOCAL_RANK_NIC_7=mlx5_5
    export CONCORD_BASE_PORT
...
run_network_node() {
    ...
    exec torchrun \
        ...
        --lanes-per-peer "$NETWORK_LANES" --base-port "$CONCORD_BASE_PORT"
}
...
remote_network_command() {
    ...
    printf '%q ' \
        ...
        "CONCORD_BASE_PORT=$CONCORD_BASE_PORT" "NETWORK_BYTES=$NETWORK_BYTES" \
    ...
    printf 'bash %q network-client' "$REMOTE_WORKDIR/pre-tests/gpt2/microbench/rdma/run_concord_microbench.sh"
}
