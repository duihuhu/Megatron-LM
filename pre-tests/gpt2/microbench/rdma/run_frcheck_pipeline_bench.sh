...
PROFILE=${PROFILE:-10b-frcheck-save}
DRY_RUN=${DRY_RUN:-0}
...
apply_profile() {
    case "$PROFILE" in
        10b-frcheck-save)
            ...
            FRCHECK_RDMA_LANES_PER_PEER=${FRCHECK_RDMA_LANES_PER_PEER:-12}
    ...
    FRCHECK_RDMA_LANES_PER_PEER=${FRCHECK_RDMA_LANES_PER_PEER:-4}
...
export_env() {
    export MASTER_ADDR MASTER_PORT FRCHECK_BASE_PORT CLUSTER_NODES PROFILE
    ...
    export FRCHECK_RDMA_LANES_PER_PEER FRCHECK_LAYER_EXCHANGE_SEG
...
remote_command() {
    ...
    for pair in \
        ...
        "FRCHECK_RDMA_LANES_PER_PEER=$FRCHECK_RDMA_LANES_PER_PEER" \
    ...
    printf 'bash %q node %q' \
        "$REMOTE_WORKDIR/pre-tests/gpt2/microbench/rdma/run_frcheck_pipeline_bench.sh" "$node_rank"
}
...
run_cluster() {
    ...
    for ((node = 1; node < CLUSTER_NODES; node++)); do
        ...
        ssh "${ssh_opts[@]}" "node$node" \
            "cd '$REMOTE_WORKDIR' && ALLOW_GPU_CONTEXT_PIDS=$(printf %q "${ALLOW_GPU_CONTEXT_PIDS:-}") bash '$REMOTE_WORKDIR/pre-tests/gpt2/microbench/rdma/run_frcheck_pipeline_bench.sh' check"
    done
