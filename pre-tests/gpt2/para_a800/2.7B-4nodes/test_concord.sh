...
TENSORBOARD_LOGS_PATH=${TENSORBOARD_LOGS_PATH:-"/workspace/Megatron-LM/logs/gpt2-2.7b-4nodes/concord"}
CHECKPOINT_PATH=${CHECKPOINT_PATH:-"/dev/shm/models/gpt2-2.7b-4nodes-concord"}
...
if [ "${PRINT_CMD:-0}" != "0" ]; then
    ...
    printf 'CONCORD_GDR=%q PYTHONPATH=%q CUDA_VISIBLE_DEVICES=%q ' \
        "$CONCORD_GDR" "$PYTHONPATH:/workspace/Megatron-LM" "$CUDA_VISIBLE_DEVICES"
