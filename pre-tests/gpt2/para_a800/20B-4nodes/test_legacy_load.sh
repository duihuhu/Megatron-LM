...
# Megatron native torch legacy checkpoint load timing script
# Usage: ./test_legacy_load.sh <node_rank> <gpu_id_0> [gpu_id_1 ...] [mode] [additional_args...]
#
# mode (optional, default: save):
#   save      - checkpoint save only (no load / recovery flags)
#   load      - load native Megatron legacy checkpoint
#   hardware  - alias of load for launch compatibility
#
# Example: ./test_legacy_load.sh 0 0 save
# Example (8 GPUs, load): ./test_legacy_load.sh 0 0 1 2 3 4 5 6 7 load
...
# ---- mode parsing (save | load, aliases: software | hardware | hardware2) ----
MODE=save
if [ -n "$1" ] && [[ "$1" =~ ^(save|load|software|hardware|hardware2)$ ]]; then
    MODE="$1"
    shift
fi
...
TENSORBOARD_LOGS_PATH=${TENSORBOARD_LOGS_PATH:-"/workspace/Megatron-LM/logs/gpt2-20b-4nodes/legacy"}
CHECKPOINT_PATH=${CHECKPOINT_PATH:-"/dev/shm/models/gpt2-20b-4nodes-legacy"}
...
case "$MODE" in
    save)
        ...
        ;;
    load|software|hardware|hardware2)
        RECOVERY_MODE_ARGS=(
...
echo "Starting Node $NODE_RANK with GPUs $CUDA_VISIBLE_DEVICES (native legacy mode=$MODE)"
