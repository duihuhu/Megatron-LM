...
if [ "$ORIG_SECOND" == "a800" ]; then
    ...
    # Fix the potential previous Connection timed out issue by using loopback and eth0
    MASTER_ADDR=127.0.0.1
    export NCCL_SOCKET_IFNAME=eth0
    export GLOO_SOCKET_IFNAME=eth0
fi
...
# Use --save to save GPU replicas and --load to reuse them
...
# -------------------------------------------------------------------------
# Move the PRINT_CMD check above the command execution
# -------------------------------------------------------------------------
