...
class CPUMemoryPool:
    ...
    def read_raw_bytes(self, address: int, size: int) -> bytes:
        """
        Read raw bytes directly from memory (zero-copy, no serialization)

        purpose:
        - Read raw data before network transmission
        - avoid torch.save/pickle serialization overhead
        - directly get memory in bytes stream
        ...
        Example:
            ...
            # Send raw_bytes (no serialization overhead)
        """
    ...
    def write_raw_bytes(self, address: int, data: bytes):
        """
        Write raw bytes directly to memory (zero-copy, no deserialization)

        purpose:
        - Write directly into memory after network reception
        - avoid torch.load/pickle deserialization overhead
        - directlywillbyte streamwritememory

        Args:
            address: target memory address
            data: byte stream to write

        Example:
            # Receive original bytes
            raw_bytes = recv_from_network()
            # Allocate memory
            address, tid = pool.allocate(len(raw_bytes))
            # write directly (no deserialization)
