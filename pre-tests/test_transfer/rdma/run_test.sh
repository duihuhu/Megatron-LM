#!/bin/bash
#
# Run RDMA latency test between two nodes
#
# This script helps coordinate running the server on one node
# and the client on another node for latency testing using RDMA.
#
# Usage:
...
DEFAULT_PORT=8888
# Packet size: 64KB is a common choice for RDMA latency testing
...
# - Large packets (128KB+): Better for throughput testing
...
check_binaries() {
    if [ ! -f "$SERVER_BIN" ]; then
        echo "Error: rdma_server not found. Please run ./build.sh first."
        exit 1
    fi
    
    if [ ! -f "$CLIENT_BIN" ]; then
        echo "Error: rdma_client not found. Please run ./build.sh first."
        exit 1
    fi
}
