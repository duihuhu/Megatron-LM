#!/usr/bin/env python3
"""Verify Concord checkpoint RS encoding across all layers and stripes."""
import struct, ctypes
from pathlib import Path
import numpy as np

BASE = Path("/dev/shm/data/checkpoint/models/gpt2-345m-0-concord/iter_0000004")
...
def rank_dir(r, use_concord_sub=False):
    d = BASE / f"mp_rank_00_{r:03d}"
    return d / "concord" if use_concord_sub else d
...
# auto-detect layout
use_sub = not (rank_dir(0) / "concord_main_rank0.pt").is_file()
if use_sub:
    print("Using OLD layout: mp_rank_*/concord/")
else:
...
for layer in LAYERS:
    ...
    for sid, row in enumerate(poa):
        ...
        paths = {
            "d0": rank_dir(src_r[0], use_sub) / layer / f"stripe_{sid}" / f"concord_shard_rank{src_r[0]}.pt",
            "d1": rank_dir(src_r[1], use_sub) / layer / f"stripe_{sid}" / f"concord_shard_rank{src_r[1]}.pt",
            "p1": rank_dir(enc_r, use_sub) / layer / f"stripe_{sid}" / f"concord_shard_rank{enc_r}_p1.pt",
            "p2": rank_dir(par_r, use_sub) / layer / f"stripe_{sid}" / f"concord_shard_rank{par_r}.pt",
        }
