#!/usr/bin/env python3
"""
Distributed Concord RDMA test (torchrun).

Usage:
  torchrun --nproc_per_node=4 tests/test_concord_dist.py
...
def concord_dist_test():
    """Test Concord RDMA pipeline with 4 processes."""
    ...
    print(f"[Rank {rank}] Starting Concord distributed test (world_size={world_size})")
    ...
    native.stop()
    dist.barrier()
    print(f"[Rank {rank}] Concord distributed test PASSED")
