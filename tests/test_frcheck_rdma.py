#!/usr/bin/env python3
"""
Standalone FRCheck RDMA test for a single process (no distributed).
...
Usage:
  python tests/test_frcheck_rdma.py
"""
...
def test_frcheck_manager():
    """Test FRCheckManager helper methods without RDMA."""
    print("=== Test FRCheckManager ===")
    from megatron.core.dist_checkpointing.strategies.frcheck_manager import (
        FRCheckManager, StripeRole
    )

    mgr = FRCheckManager()
    # Test group layout for world_size=4, n=4
    layout = FRCheckManager._get_group_layout(4, 4)
    assert layout['num_groups'] == 1, f"Expected 1 group, got {layout['num_groups']}"

    for rank in range(4):
        gid = FRCheckManager._get_group_id(rank, 4, 4)
        rig = FRCheckManager._get_rank_in_group(rank, 4, 4)
        restored = FRCheckManager._get_rank_by_group_position(gid, rig, 4, 4)
    ...
    native = native_mod.FRCheckNative(POA_PATH)
    for r in range(4):
        n2 = native_mod.FRCheckNative(POA_PATH)
        n2.compile_plans(r)
        ns = n2.num_stripes()
...
def test_poa_autogenerate():
    ...
    for n in [3, 4, 5, 7, 8, 11, 16]:
        native = native_mod.FRCheckNative(n)
